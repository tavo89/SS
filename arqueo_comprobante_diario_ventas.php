...
$cols="anticipo_bono,id_cli,tipo_venta,prefijo as pre,nom_cli,snombr,apelli,anulado, num_fac_ven, SUM(sub_tot) sub_tot, iva,SUM(IFNULL(($excento_art),0)) as excento,SUM(tot) tot,vendedor, TIME(fecha) as hora, DATE(fecha) as fe, tipo_venta,tipo_cli,SUM($sub16_art) as sub16,SUM($sub05_art) as sub05,SUM($sub10_art) as sub10,SUM($sub19_art) as sub19, COUNT(*) AS nf";
	...
	$('.tablaDataTables').DataTable({
        ...
        { type: 'numeric-comma', targets: 2 }],*/
			language: {url: 'locales/es.json'},
				...
				<?php if($fechaI<"2017-01-31")
				{echo "doc.content[1].table.widths = [ '8%', '10%', '10%', '8%', '7%', '7%', '7%', '7%', '7%', '7%', '10%', '9%', '9%', '8%'];";}
				else {echo "doc.content[1].table.widths = [ '8%', '8%', '8%', '8%', '10%', '10%', '7%', '7%', '10%', '10%', '10%', '8%'];";} ?> 
