...
$rs=$linkPDO->query("SELECT * from fac_venta WHERE nit=$codSuc AND tipo_venta='Credito' AND ".VALIDACION_VENTA_VALIDA." AND estado='PAGADO' $A $B GROUP BY id_cli order by 3");
$i=0;
while($row=$rs->fetch())
{
...
$sql="SELECT * from fac_venta WHERE nit=$codSuc AND tipo_venta='Credito' AND ".VALIDACION_VENTA_VALIDA." AND id_cli!='$NIT_FANALCA' $A $B GROUP BY num_fac_ven,prefijo";
...
<span align="center" style="font-size:24px"><B>CREDITOS CANCELADOS <?php //echo $B ?></B></span></p>
</td>
...
for($i=0;$i<count($id_clientes);$i++)
{
	$tot_saldo=0;
	$tot_abono=0;
	$sql="SELECT *,DATEDIFF(CURRENT_DATE(),DATE(fecha) ) AS mora,DATEDIFF(DATE(fecha_pago),DATE(fecha) ) AS mora2 FROM fac_venta WHERE id_cli='".$id_clientes[$i]."' AND ".VALIDACION_VENTA_VALIDA." AND tipo_venta='Credito' AND estado='PAGADO' $A $B";
	$rs=$linkPDO->query($sql);
	?>
<tr>
<td colspan="6"><b><?php echo $id_clientes[$i]." - ".$nom_clientes[$i] ?></b></td>
 ...
 <td><?php echo $mora ?></td>
	...
	?>
<tr>
<tH colspan="3"><b>TOTALES</b></th><td><?php echo money3($tot_abono) ?></td><td><?php  echo money3($tot_saldo)?></td><td></td>
</tr> 
