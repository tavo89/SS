...
if(isset($_SESSION['clases'])&&!empty($_SESSION['clases']))
{
	...
	$filtroClase="AND (";
	foreach($Clase as $key=> $resultado)
	{
		$filtroClase.="id_clase='$resultado' OR ";
	}
	$filtroClase.=" id_clase='$resultado') ";
	if(empty($resultado))$filtroClase="";
	}
$filtroFab="";
if(isset($_SESSION['fabs'])&&!empty($_SESSION['fabs']))
{
	$fabricante=$_SESSION['fabs'];
	$filtroFab="AND (";
	foreach($fabricante as $key=> $resultado)
	{
		$filtroFab.="fab='$resultado' OR ";
	}
	$filtroFab.=" fab='$resultado') ";
	if(empty($resultado))$filtroFab="";
	}
...
<div style=" top:0cm; width:21.5cm; height:27.9cm; position:absolute;padding-left:20px;">
...
<span align="center"><B>Informe de Ventas por Producto</B></span>
</p>
</td>
...
if(!empty($_SESSION['nom_cli_inf2'])){$cli=$_SESSION['nom_cli_inf2'];echo "<b>Cliente:</b> $cli <br>";}
...
<table frame="box" rules="all" cellpadding="0" cellspacing="1" width="100%"   class="display dataTable table table-striped table-bordered table-hover tablaDataTables">
<thead>
<tr>
<th>Ref.</th>
<th>Cod.</th>
...
$TOT_VENTAS_SIN=0;
$TOT_VENTAS=0;
$TOT_COSTO_VENDIDOS=0;
$TOT_COSTO_VENDIDOS_IVA=0;
$TOT_UTILIDAD=0;

$FacTor="( (a.unidades_fraccion+(a.cant*a.fraccion))/a.fraccion )";
...
FROM art_fac_ven a
LEFT JOIN productos b ON REPLACE(a.ref,'old','') = b.id_pro"; 
	
	$sql="SELECT a.fraccion,
	SUM(a.unidades_fraccion) AS unidades_fraccion,
	a.fab,b.ciudad,
	b.nit, 
	b.id_cli,
	b.nom_cli,
	...
	SUM(a.cant) AS cant,
	a.iva as IVAart,
	avg(a.precio) as precio,
	SUM(a.precio*$FacTor) as  art_sub,
	SUM(a.costo*$FacTor) AS totCost,
	a.dcto,
	a.costo,
	b.num_fac_ven,
	b.prefijo,
	...
	ON a.num_fac_ven=b.num_fac_ven  AND a.prefijo=b.prefijo AND b.nit=a.nit  AND b.".VALIDACION_VENTA_VALIDA." 
	WHERE  b.nit=$codSuc  $filtroFormaPago  $A $C $E  $F $filtroClase $filtroFab 
	GROUP BY a.cod_barras";
	...
	$pvp = $row["precio"]*1;
	$costo=$row['costo'];
	$totCosto=$factor*$costo;
	$totCostoIVA= $totCosto*(($iva/100) +1);
	$totCant+=$factor;
	
	$iva05= ($iva==5) ?round($factor*$costo*($iva/100),0):0;
	$iva10= ($iva==10)?round($factor*$costo*($iva/100),0):0;
	$iva19= ($iva==19)?round($factor*$costo*($iva/100),0):0;
	
	$totIva05= ($iva==5) ?$totIva05+$iva05:$totIva05;
	$totIva10= ($iva==10)?$totIva10+$iva10:$totIva10;
	$totIva19= ($iva==19)?$totIva19+$iva19:$totIva19;
	...
	$art_sub=redondeo($row["art_sub"]*1);
		...
		$TOT_ABONO+=$abono;
		$TOT_SALDO+=$art_sub;
		$TOT_COSTO_VENDIDOS+=$totCosto;
		$TOT_COSTO_VENDIDOS_IVA+=$totCostoIVA;
		$TOT_VENTAS+=$art_sub;
 ...
 <td><?php echo money2($totCosto) ?></td>
   <td><?php echo money2($totCosto*($iva_art+1)) ?></td>
   <td><?php echo $iva05 ?></td>
   <td><?php echo $iva10 ?></td>
   <td><?php echo $iva19 ?></td>
 <td><?php echo  ($art_sub)/*."<BR> COSTO : $costo <BR>FACTOR($factor): (($uni/$frac)+$cant) <BR> totCostoIVA ($totCostoIVA): $totCosto*(($iva/100) +1)"    */ ?></td>
  <td><?php echo util($art_sub,$totCosto,$iva,"per") ?>%</td>
...
<td></td>
<td></td>
<td><?php echo $totCant; ?></td>

<td><?php  echo money3(round($TOT_COSTO_VENDIDOS,0))?></td>
<td><?php  echo money3(round($TOT_COSTO_VENDIDOS_IVA,0))?></td>
<td><?php  echo money3($totIva05)?></td>
<td><?php  echo money3($totIva10)?></td>
<td><?php  echo money3($totIva19)?></td>
<td><?php  echo money3($TOT_SALDO)?></td>
<td><?php echo redondeo2(util_tot($TOT_VENTAS,$TOT_COSTO_VENDIDOS_IVA,"per")) ?>%</td>
<td><?php  echo money3(round($TOT_UTILIDAD,0))?></td>
</tfoot>
...
$tituloInforme="Utilidad x Producto";
?>
...
$(document).ready(function()
	{
		...
		$('.tablaDataTables').DataTable({
			...
			language: {url: 'locales/es.json'},
				...
				doc.content[1].table.widths = [ '*', '*', '*', '5%', '15%', '15%', '8%', '15%'];
