...
if($row=$rs->fetch()){
			...
			if($row=$rs->fetch()){
			//$NUM_EXP=$row['num_exp'];
			$qryA="UPDATE fac_remi SET estado='',anulado='', fecha_anula='0000-00-00', usu='$nomUsu',id_usu='$id_Usu',modifica='$nomUsu RECOVER FAC. $pre-$num_fac' WHERE num_fac_ven=$num_fac AND prefijo='$pre' AND nit=$codSuc";
			t1($qryA);
...
SET i.exist=(i.exist-a.cant), i.unidades_frac=(i.unidades_frac-a.unidades_fraccion) WHERE i.nit_scs=a.nitAr and i.nit_scs=$codOrigen AND i.fecha_vencimiento=a.fecha_vencimiento AND i.id_pro=a.ref";
t2($qryA,$qryB);
}
else{t1($qryA);
}
	...
	$HTML_antes="";
	$HTML_despues="<div style='font-size:24px;'>FACTURA <span style='color:red'>No. $num_fac - $pre</span> <b>RECUPERADA</b></div>";
