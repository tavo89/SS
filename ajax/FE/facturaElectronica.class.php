...
function formatoNumFactura($prefijo, $numFac){
    /*  validacion correspondiente a prefijos vacíos, en este sistema no se pueden dejar vacios,
        pero en la DIan SÍ.
    */
    $formato= ($prefijo=='----')?$numFac:$prefijo.$numFac;
    return $formato;
}

function initHeader(){
    global $tokenDianOperaciones,$usuarioDian,$nit_reg_dian;

    $header =array( "auth-token:".$tokenDianOperaciones,
...
function getFacturaData(){
    ...
    define("serial_fac" , r('serial_fac'));
    define("num_fac" , r('num_fac'));
    define("prefijo" , r('prefijo'));
    define("codSuc" , r('codSuc'));
    define("hash" , r('hash'));

    $filtroHash=" AND hash='".hash."'";
    if(empty($hash) || $hash==0)$filtroHash="";

    $sql="SELECT estado_factura_elec,mail FROM fac_venta 
    ...
    if($row=$rs->fetch()){
        ...
        $correoCliente = $row['mail'];
    }   
    return array('estadoDian'=>$estadoDian,
                 'correoCliente'=>$correoCliente); 
}
function consultaIdFacDian($numeroFac){
    $urlApi='https://api.dataico.com/direct/dataico_api/v2/invoices?number='.$numeroFac;
    $RespApi=consultarCamposDian($urlApi);
   
    return json_decode($RespApi,true);
}
function actualizaEstadoFactura($params){
    global $linkPDO;
    $sql = "UPDATE fac_venta SET 
    estado_factura_elec = '".$params['estadoFac']."',
    resp_dian = '".$params['dian_status']."',
    cufe = '".$params['cufe']."'
    WHERE num_fac_ven='".$params['numeroFac']."' AND prefijo='".$params['prefijoFac']."'
	          AND nit='".$params['sucursal']."' ";
    t1($sql);

}
function reEnviaCorreo($correo, $idFactura){
    $url ='https://api.dataico.com/direct/dataico_api/v2/invoices/'.$idFactura;
    $data = array('actions'=>array('send_email'=>true,
                                   'email'=>$correo));
...
function reEnviarFacturaOmail($urlApi,$data, $transformaParams){

    $params = ($transformaParams==true) ?json_encode($data) : $data;
	$curl = curl_init();
    
    /*echo PHP_EOL;
    echo "endPoint: $urlApi";
    echo PHP_EOL;
    echo "Params : $params";*/
...
function consultarCamposDian($urlApi){
 ...
 *
 */
function enviaDocumentoDian($urlApi,$camposFactura, $transformaParams = true){
	global $company_fe,$usuarioDian,$nit_reg_dian,$tokenDianOperaciones;
//	echo "transformaParams : $transformaParams";
	...
	if($company_fe!='DATAICO'){
        $header = array(
            "Authorization: Bearer ".$tokenDianOperaciones,
...
function actualizaFechaFE($num_fac,$prefijo,$codSuc,$fechaFactura,$horaFactura) {
    global $linkPDO,$FechaHoy;
    if($fechaFactura!=$FechaHoy) {
    $sql = "UPDATE fac_venta SET fecha ='$FechaHoy $horaFactura' 
            WHERE num_fac_ven='$num_fac' AND prefijo='$prefijo' AND nit='$codSuc'";
            //echo "$sql";
    $linkPDO->exec($sql);
    }
}
 ...
 */
 ...
 {
    global $linkPDO,$MODULES,$company_fe,$dataico_account_id,$send_dian,$usar_iva;
	...
	if($row=$rs->fetch()){
        ...
        // depurar datos NIT,correo, direccion
        $id_cli = str_replace('.','',$row['id_cli']); // quitar todo lo no numeroci, y dejar el "-"
        $findme ='-';
		$pos = strpos($id_cli, $findme);// verifica si encuntra un "-" (es NIT)
		$nitCli = $id_cli;
        $sanitizedEmail = filter_var($row["mail"], FILTER_SANITIZE_EMAIL);
        $direccionCliente = !empty($row['dir']) ? $row['dir']:'NO OTORGADA';
        ...
        if (!filter_var($sanitizedEmail, FILTER_VALIDATE_EMAIL)) {
            $sanitizedEmail = 'facturacion_dian@nanimosoft.com';
        }
        
        // si encuentra un "-", depurar NIT para FE
		if ($pos !== false) {
    ...
    $SUB=0;
    $camposFactura.= '"items": [';
    $sql = "SELECT *,fecha_vencimiento as feven 
            FROM art_fac_ven  
            WHERE num_fac_ven='$num_fac' AND prefijo='$prefijo' AND  nit='$codSuc' $filtroHash ORDER BY orden_in";
            //echo "<br>$sql ";
	$rs=$linkPDO->query($sql );
	$numRows=$rs->rowCount();
	$i=0;
    ...
    $i++;
    $iva = $usar_iva==1?(int)$row["iva"]:0;
    $ivaDepurado = ($iva==19 || $iva==16 || $iva==10 || $iva==5 || $iva==0 )? $iva:19;
    $iva = $ivaDepurado;
