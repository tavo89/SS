...
$url="lista_fichas_colorimetria.php";
...
$sql = "SELECT  $columns FROM ficha_tecnica_cabello WHERE nit=$codSuc $adminFilter $A $C $E ORDER BY fecha DESC,num_ficha DESC LIMIT $offset, $limit"; 
...
$sqlTotal = "SELECT COUNT(*) as total FROM ficha_tecnica_cabello WHERE nit='$codSuc' $adminFilter $A $C $E"; 
...
$sql_busq="SELECT $columns FROM ficha_tecnica_cabello  WHERE (id_cli='$busq'  OR nom_cli LIKE '$busq%' OR num_fac_ven='$busq' LIKE '$busq%')  AND (nit=$codSuc $adminFilter $B $A $C $E)";
...
?>
<h1 align="center">FICHAS TECNICAS DE COLORIMETRIA Y TEXTURIZACION</h1>
<br><br><br>
<!--
...
?>
<td>
<a href="editar_ficha_colorimetria.php?ficha=<?php echo $num_ficha ?>&pag=<?php echo $pag ?>" class="uk-icon-pencil uk-icon-button uk-icon-hover uk-icon-small">
<!--
...
<a onclick="window.open('imp_ficha_colorimetria.php?ficha=<?php echo $num_ficha; ?>','Ficha No. <?php echo $num_ficha; ?>','width=1200px,height=600px,scrollbars=YES, location = YES menubar = NO, status = NO, titlebar = NO, toolbar = NO, resizable = YES , directories = NO');" class="uk-icon-print uk-icon-button uk-icon-hover uk-icon-small"> 
