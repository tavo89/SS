...
$TOT_VENTAS_PVP=0;
$rs=$linkPDO->query("SELECT * from fac_venta WHERE nit=$codSuc AND ".VALIDACION_VENTA_VALIDA." $filtroFormaPago $B $A $E $F GROUP BY id_cli order by 3");
$i=0;
while($row=$rs->fetch())
{
...
$TOT_UTILIDAD_VENTAS=tot_utilidad($A,$B,$C,$E,$F,$filtroFormaPago,$codSuc);

$TOT_COSTO_SIN[]=$TOT_UTILIDAD_VENTAS["TOT_COSTO_SIN"];
$TOT_COSTO_IVA[]=$TOT_UTILIDAD_VENTAS["TOT_COSTO_IVA"];
$TOT_VENTAS_COSTO_IVA=$TOT_UTILIDAD_VENTAS["TOT_VENTAS_COSTO_IVA"];
$TOT_VENTAS_PVP=$TOT_UTILIDAD_VENTAS["TOT_VENTAS_PVP"];

$UTIL[]=$TOT_UTILIDAD_VENTAS["UTIL"];
$TOT_UTIL=$TOT_UTILIDAD_VENTAS["TOT_UTIL"];
...
<span align="center"><B>INGRESOS USUARIOS REGISTRADOS</B></span>
</p>
</td>
...
$sql="SELECT SUM(b.iva) iva,b.nit, b.id_cli,b.nom_cli,SUM(b.tot) as tot,b.fecha,SUM(b.descuento) descuento,b.vendedor,COUNT(*) as totF,DATEDIFF(NOW(),MIN(fecha)) as dias FROM  fac_venta b  WHERE b.nit=$codSuc AND b.".VALIDACION_VENTA_VALIDA." $filtroFormaPago $A $C $E $B $F GROUP BY id_cli $ORDER_BY";
	...
	$Sql2="SELECT * FROM usuarios WHERE id_usu='$idCli' ";
...
<!--<td><?php echo money2($descuento+$art_sub) //echo money2($factor*$refPrecio[$ref])?></td>-->
<td><?php  echo money2($iva)?></td>
<td><?php  echo ($tot_saldo)?></td>
<?php
if($ver_util==10){
?>
<td><?php echo util_tot($tot_saldo,$TOT_COSTO_IVA[$idCli],"per"); ?></td>
<td><?php  echo money2(util_tot($tot_saldo,$TOT_COSTO_IVA[$idCli],"tot"))?></td>
<?php
}
?>
...
<td><?php  echo money3(util_tot($TOT_VENTAS_PVP,$TOT_VENTAS_COSTO_IVA,"tot")) ?></td>
<?php
}
?>
		...
		$('.tablaDataTables').DataTable({
			language: {url: 'locales/es.json'},
                ...
                buttons: [
                    { extend: 'copy'},
                    {extend: 'csv'},
                    {extend: 'excelHtml5', title: 'Informe Ingresos '+rangoFecha,footer:true, customize: function ( xlsx ){
                ...
                $('row c[r^="F"]', sheet).each( function () {
                    ...
                    {extend: 'pdf', title: 'Informe Ingresos '+rangoFecha,footer:true,orientation:'landscape',pageSize:'letter',
