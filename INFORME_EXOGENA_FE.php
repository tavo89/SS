...
$NOMBRE_INFORME="$NOM_SUCURSAL NIT: $NIT_NEGOCIO - VENTAS ";
$url=thisURL();
$boton=r("boton");
...
$sql="SELECT  nit,COUNT(num_fac_ven) as tot_fac,MAX(num_fac_ven) as ultima,MIN(num_fac_ven) as primera FROM fac_venta  WHERE num_fac_ven NOT IN(SELECT num_fac_ven FROM fac_venta  WHERE  nit=$codSuc  AND DATE(fecha_anula)>='$fechaI' AND DATE(fecha_anula)<='$fechaF')  AND nit=$codSuc AND DATE(fecha)>='$fechaI' AND DATE(fecha)<='$fechaF' AND ".VALIDACION_FAC_ELECTRONICA_VALIDA."";
...
<?php
$filtroCerradas=" AND ".VALIDACION_FAC_ELECTRONICA_VALIDA."";
        ...
        { type: 'numeric-comma', targets: 2 }],*/
			language: {url: 'locales/es.json'},
                ...
                buttons: [
                    { extend: 'copy'},
                    {extend: 'csv'},
                    {extend: 'excelHtml5', title: '<?php echo "Exogena Factura electronicas";?> '+rangoFecha, customize: function ( xlsx ){
                ...
                $('row c[r^="F"]', sheet).each( function () {
                    ...
                    {extend: 'pdf', title: '<?php echo "Exogena Factura electronicas ";?> '+rangoFecha ,footer:true,orientation:'landscape',pageSize:'letter',
