...
<title>FACTURAS CON ANTICIPOS</title>
...
<div class="Section1" style=" top:0cm; padding:20px;">
<table align="center" width="100%">
<tr>
 
<td valign="top" colspan="3">
<p align="left" style="font-size:12px;">
<span align="center" style=" font-size:24px"><B>FACTURAS CON ANTICIPO</B></span>
</p>
</td>
...
$cols="abono_anti,anticipo_bono,id_cli,tipo_venta,prefijo as pre,nom_cli,snombr,apelli,anulado, num_fac_ven, sub_tot, iva,IFNULL(($excento_art),0) as excento,tot,vendedor, TIME(fecha) as hora, DATE(fecha) as fe, tipo_venta,tipo_cli,($sub16_art) as sub16,($sub05_art) as sub05";

//$sql="SELECT $cols FROM fac_venta WHERE  nit = $codSuc   AND DATE(fecha)>='$fechaI' AND DATE(fecha)<='$fechaF'  ORDER BY fecha";
$sql="SELECT $cols FROM fac_venta, sucursal WHERE sucursal.cod_su =$codSuc AND fac_venta.nit = sucursal.cod_su AND num_exp!=0  AND DATE(fecha)>='$fechaI' AND DATE(fecha)<='$fechaF'  ORDER BY fecha";
//echo "$sql";
...
while($row=$rs->fetch())
{
	...
	if($SUB16!=0 || $IVA16!=0 || $excento!=0 || $TOTAL){
	?>
    <tr>
    ...
    -->
     <td><?php echo money_dt($abonAnti) ?></td>
...
?>
    ...
    <?php
        $base_tot=redondeo(redondeo((($TOTAL-$excentas)/1.19 /*+ $excentas*/)));
        $iva_tot=redondeo(($TOTAL-$excentas)-($TOTAL-$excentas)/1.19);
...
<tfoot id="asd11">
    ...
    <td>
        <span ><?PHP echo money_dt(redondeo($TOTAL)) ?></span>
    </td>
    <td>
    ...
    <tr>
        <td colspan="7" rowspan="1" style="font-size:22px;text-align: center;color:white;">Total Ventas</td>
        <td colspan="3" rowspan="1" style="font-size:22px;text-align: center;color:white;"><?PHP echo money_dt(redondeo($TOTAL)) ?></td>
    </tr>
   </tfoot>
</table>
<hr align="center" width="100%" />
        ...
        { type: 'numeric-comma', targets: 2 }],*/
			language: {url: 'locales/es.json'},
                ...
                buttons: [
                    { extend: 'copy'},
                    {extend: 'csv'},
                    {extend: 'excelHtml5', title: 'FAC. ANTICIPOS '+rangoFecha, customize: function ( xlsx ){
                ...
                $('row c[r^="F"]', sheet).each( function () {
                    ...
                    {extend: 'pdf', title: 'FAC. ANTICIPOS '+rangoFecha ,orientation:'landscape',pageSize:'letter',footer:true,
                ...
                // Data URL generated by http://dataurl.net/#dataurlmaker
                    ...
                    {extend: 'print',tittle:'Facutas con Anticipo',
