<?php
include_once("Conexxx.php");
$fechaI=$_SESSION['fechaI'];
$fechaF=$_SESSION['fechaF'];
$NOMBRE_INFORME="VENTAS ";
$url=thisURL();
$boton=r("boton");
if($boton=="MS EXCEL"){excel("Cartera Clientes");}
//excel("Cartera Clientes");
$horaI=s("horaI");
$horaF=s("horaF");
$CodCajero=s("cod_caja_arq");
$base_iva16=0;

$iva16=0;
$base_iva5=0;
$iva5=0;
$excentas=0;
$base_iva16B=0;
$iva16B=0;
$base_iva5B=0;
$iva5B=0;
$excentasB=0;
$ult_fac=0;
$pri_fac=0;
$DCTO=0;
$tot_comprobantes=0;

$tot_fac_taller=0;
$tot_fac_inspecciones=0;
$tot_fac_otros_talleres=0;
$tot_fac_mostrador=0;

$tot_fac_contado=0;
$tot_fac_credito=0;
$tot_fac_tarjeta_credito=0;
$tot_fac_cheque=0;
$InteresesCreditos=0;

$TOTAL=0;




//***********************************************************************************************************************************************************************
//***********************************************************************************************************************************************************************
//***********************************************************************************************************************************************************************


$sql="SELECT  nit,COUNT(num_fac_ven) as tot_fac,MAX(num_fac_ven) as ultima,MIN(num_fac_ven) as primera FROM fac_venta  WHERE num_fac_ven NOT IN(SELECT num_fac_ven FROM fac_venta  WHERE  nit=$codSuc  AND DATE(fecha_anula)>='$fechaI' AND DATE(fecha_anula)<='$fechaF')  AND nit=$codSuc AND DATE(fecha)>='$fechaI' AND DATE(fecha)<='$fechaF' ";

$rs=$linkPDO->query($sql);


if($row=$rs->fetch())
{
	$tot_fac=$row['tot_fac'];
	$ult_fac=$row['ultima'];
	$pri_fac=$row['primera'];
}

?>

<!DOCTYPE html PUBLIC >
<html xmlns:v="urn:schemas-microsoft-com:vml"
xmlns:o="urn:schemas-microsoft-com:office:office"
xmlns:x="urn:schemas-microsoft-com:office:excel"
xmlns="http://www.w3.org/TR/REC-html40">
<head>
<link href="font-awesome/css/font-awesome.css" rel="stylesheet">
<link href="css/animate.css" rel="stylesheet">
<link href="css/style.css" rel="stylesheet">
<link href="css/plugins/dataTables/datatables.min.css" rel="stylesheet">
<link href="css/bootstrap.min.css" rel="stylesheet">
    
<meta http-equiv=Content-Type content="text/html; charset=<?php echo $CHAR_SET ?>">
<meta name=ProgId content=Excel.Sheet>
<meta name=Generator content="Microsoft Excel 11">

<title><?php echo "$NOMBRE_INFORME";?></title>

<?php include_once("IMP_HEADER.php"); ?>


<style >
 body{
	
transform: scale(0.8);
transform-origin: top left;
-moz-transform: scale(0.8);
-moz-transform-origin: top left;
}
</style>



</head>

<body style="font-size:12px">
<div class="Section1" style=" top:0cm; width:21.5cm; height:27.9cm; position:absolute;">
<table align="center" width="100%">
<tr>

<td valign="top" colspan="3">
<p align="left" style="font-size:12px;">
<span align="center" style=" font-size:24px"><B>INFORME FACTURAS DE VENTAS</B></span>
</p>
</td>

</tr>
</table>
Fecha: <?PHP echo $hoy ?>
<br>
<table align="left" width="">
<tr style="font-size:24px; font-weight:bold;">
<td >
Desde: <?PHP echo $_SESSION['fechaI'] ?>
</td>
<td > Hasta: <?PHP echo $_SESSION['fechaF'] ?>
</td>
</tr>
<tr>
<td colspan="">
<table cellspacing="0px" cellpadding="0px">
<tr>
<td><b>Cajero </b></td><td width="50px"></td><td colspan=""><b><?php echo $_SESSION['nom_usu'] ?></b></td>
</tr>
</table>
</td>
</tr>

<tr>
<td>
<b>Desde Fac No. <?php echo $pri_fac ?></b>
</td>
<td>
<b>Hasta Fac No. <?php echo $ult_fac ?></b>
</td><td><b>Total Fac. <?php echo $tot_fac ?></b></td>
</tr>
</table>


<hr align="center" width="100%">
<table frame="box" rules="all" cellpadding="5" cellspacing="3" width="100%" class="display dataTable table table-striped table-bordered table-hover tablaDataTables" style="font-size:10px;">
<thead>
<tr style="font-size:12px; ">
<th>#Fac.</th>
<th width="120px" colspan="">Cliente</th>
<th>C.C/NIT</th>
<th width="120px" colspan="">Direcci&oacute;n</th>
<th>Barrio</th>
<th>Plan</th>
<th>Tipo</th>
<th>Estrato</th>
<!--<th>Forma pago</th>-->

<th>SUB 19%</th><th>IVA 19%</th>
<?php if($fechaI<"2017-01-31"){?> 
<th>SUB 16%</th><th>IVA 16%</th>
<?php } ?> 
<!--<th>SUB 5%</th><th>IVA 5%</th>-->

<th>Exento IVA</th>
<th>Excluido IVA</th>
<th>Intereses</th>
<th>Afiliaci&oacute;n Exenta</th>
<th>Afiliaci&oacute;n Excluida</th>
<th>Afiliaci&oacute;n Gravada</th>
<th>Saldo Anterior</th>
<th>Descuento</th>
<th>Total</th>
<!--
<th>Vendedor</th>
-->
<th width="60">Fecha</th><th>Estado</th>

</tr>
</thead>
<tbody>

<?php
$filtroCerradas=" AND anulado='CERRADA'";
$filtroCerradas=" ";


$excento_art="(select SUM(sub_tot) from art_fac_ven where art_fac_ven.num_fac_ven=fac_venta.num_fac_ven and iva=0 and art_fac_ven.nit=fac_venta.nit AND art_fac_ven.prefijo=fac_venta.prefijo  )";


$sub16_art="(select SUM(sub_tot/1.16) from art_fac_ven where art_fac_ven.num_fac_ven=fac_venta.num_fac_ven and iva=16 and art_fac_ven.nit=fac_venta.nit AND art_fac_ven.prefijo=fac_venta.prefijo )";


$sub05_art="(select SUM(sub_tot/1.05) from art_fac_ven where art_fac_ven.num_fac_ven=fac_venta.num_fac_ven and iva=5 and art_fac_ven.nit=fac_venta.nit AND art_fac_ven.prefijo=fac_venta.prefijo )";

$sub10_art="(select SUM(sub_tot/1.10) from art_fac_ven where art_fac_ven.num_fac_ven=fac_venta.num_fac_ven and iva=10 and art_fac_ven.nit=fac_venta.nit AND art_fac_ven.prefijo=fac_venta.prefijo )";

$sub19_art="(select SUM(sub_tot/1.19) from art_fac_ven where art_fac_ven.num_fac_ven=fac_venta.num_fac_ven and iva=19 and art_fac_ven.nit=fac_venta.nit AND art_fac_ven.prefijo=fac_venta.prefijo )";


 

$cols="descuento,anticipo_bono,id_cli,tipo_venta,prefijo as pre,nom_cli,snombr,apelli,anulado, num_fac_ven, SUM(sub_tot) sub_tot, iva,SUM(IFNULL(($excento_art),0)) as excento,SUM(tot) tot,vendedor, TIME(fecha) as hora, DATE(fecha) as fe, tipo_venta,tipo_cli,SUM($sub16_art) as sub16,SUM($sub05_art) as sub05,SUM($sub10_art) as sub10,SUM($sub19_art) as sub19, COUNT(*) AS nf,dir,regFiscal";

 

//$sql="SELECT $cols FROM fac_venta WHERE  nit = $codSuc   AND DATE(fecha)>='$fechaI' AND DATE(fecha)<='$fechaF'  ORDER BY fecha";
$sql="SELECT $cols FROM fac_venta, sucursal WHERE sucursal.cod_su =$codSuc AND fac_venta.nit = sucursal.cod_su  AND DATE(fecha)>='$fechaI' AND DATE(fecha)<='$fechaF' $filtroCerradas GROUP BY  num_fac_ven   ORDER BY fecha";
//echo "$sql";
$rs=$linkPDO->query($sql);
$tot_mostrador=0;
$tot_fac_mostrador=0;
$TOTAL=0;


$tot_tarjetaDeb=0;
$tot_contado=0;
$tot_Credito=0;
$tot_tarjetaCre=0;
$tot_cheque=0;
$excluidos=0;


while($row=$rs->fetch())
{
$valorAfiliacion=0;
$valorAfiliacionExcluida=0;
$valorAfiliacionExenta=0;
$valorAfiliacionGravada=0;
$InteresMora=0;
	$pre=$row['pre'];
	$num_fac=$row['num_fac_ven'];
	$subTot=$row['sub_tot']*1;
	$IVA=$row['iva']*1;
	$excento=$row['excento']*1;
	$total=$row['tot']*1;
	$vendedor=$row["vendedor"] ;
	$HORA=$row['hora'];
	$fecha=$row['fe'];
	$tipo_venta=$row['tipo_venta'];
	$tipoCli=$row['tipo_cli'];
	$nomCli="$row[nom_cli] $row[snombr] $row[apelli]";
	$formaPago=$row['tipo_venta'];
	$idCli=$row["id_cli"];
	
	$DESCUENTO=$row["descuento"];
	if($row["descuento"]<0){$DESCUENTO="";}
	
	$direccion=$row["dir"];
	$barrio=$row["loccli"];
	
	$printAnticipo="";
	if($row["anticipo_bono"]=="SI"){$printAnticipo="<br>(Anticipo)";}
	
	
	
	
	$SERV=serv_0_05_16PlanesInter($num_fac,$pre);
	if($row['anulado']=="ANULADO"){$total=0;}
	$TOTAL+=$total;
	
	$SUB19=round($row['sub19']+$SERV[19]);
	$IVA19=round(($row['sub19']+$SERV[19])*0.19);
	
	$SUB10=round($row['sub10']+$SERV[10]);
	$IVA10=round(($row['sub10']+$SERV[10])*0.10);
	
	$SUB16=round($row['sub16']+$SERV[16]);
	$IVA16=round(($row['sub16']+$SERV[16])*0.16);
	$SUB05=round($row['sub05']+$SERV[5]);
	$IVA05=round(($row['sub05']+$SERV[5])*0.05);
	
	$excluidos=$SERV[-1];
	
	
	$excento=$SERV[0];
	
	if($excluidos==0){$excluidos="";}
	if($excento==0){$excento="";}
	
	if($SUB19==0){$SUB19="";}
	if($IVA19==0){$IVA19="";}
	if($SUB10==0){$SUB10="";}
	if($IVA10==0){$IVA10="";}
	if($SUB16==0){$SUB16="";}
	if($IVA16==0){$IVA16="";}
	if($SUB05==0){$SUB05="";}
	if($IVA05==0){$IVA05="";}
	
	$plan="";
	$tipoPlan="";
	$estrato="";
	
	$sqlAux="select * FROM serv_fac_ven WHERE num_fac_ven='$num_fac' AND prefijo='$pre' AND cod_su=$codSuc AND estrato!='' LIMIT 1";
	$rsAux=$linkPDO->query($sqlAux);
	if($rowAux=$rsAux->fetch()){
		$plan=$rowAux["anchobanda"];
		$tipoPlan=$rowAux["tipo_cliente"];
		$estrato=$rowAux["estrato"];
	}
	
	// sum intereses
	$sqlAux="select * FROM serv_fac_ven WHERE num_fac_ven='$num_fac' AND prefijo='$pre' AND cod_su=$codSuc AND serv LIKE '%INTERES%' LIMIT 1";
	$rsAux=$linkPDO->query($sqlAux);
	
	if($rowAux=$rsAux->fetch()){
		 $InteresMora=$rowAux["pvp"]*1;
		
	}
	 if($InteresMora==0){$InteresMora="";}
	
	// tot afiliaciones x fac.
	$sqlAux="select * FROM serv_fac_ven WHERE num_fac_ven='$num_fac' AND prefijo='$pre' AND cod_su=$codSuc AND iva=0 AND serv LIKE '%AFILIACION%' LIMIT 1";
	$rsAux=$linkPDO->query($sqlAux);
	
	if($rowAux=$rsAux->fetch()){
		//echo "<li>$sqlAux ---> SIN IVA</li>";
		 $valorAfiliacion=$rowAux["pvp"]*1;
		 if($excluidos!=0){$valorAfiliacionExcluida=$rowAux["pvp"]*1;$excluidos="";}
		 if($excento!=0){$valorAfiliacionExenta=$rowAux["pvp"]*1;$excento="";}
		 

		 
	}
			 if($valorAfiliacionExcluida==0){$valorAfiliacionExcluida="";}
		 if($valorAfiliacionExenta==0){$valorAfiliacionExenta="";}
		 if($valorAfiliacion==0){$valorAfiliacion="";}
	
	
		// tot afiliaciones gravadas x fac.
	$sqlAux="select * FROM serv_fac_ven WHERE num_fac_ven='$num_fac' AND prefijo='$pre' AND cod_su=$codSuc AND iva!=0 AND serv LIKE '%AFILIACION%' LIMIT 1";
	$rsAux=$linkPDO->query($sqlAux);
	
	if($rowAux=$rsAux->fetch()){
		 $valorAfiliacionGravada=$rowAux["pvp"]/(($rowAux["iva"]/100)+1);
	
		 $SUB19="";
	}
		 if($valorAfiliacionGravada==0){$valorAfiliacionGravada="";}
	
	
	// saldo periodo anterior
 
$saldoPeriodoAnt=0;

$sql2="SELECT *,DATE(fecha) AS fecha2 FROM comprobante_ingreso WHERE id_cli='$idCli' ORDER BY fecha DESC LIMIT 1";
$rs2=$linkPDO->query($sql2);
if($row2=$rs2->fetch()){
$FechaC=$row2["fecha2"];
$num_comp=$row2["num_com"];
$STATS_PAGOS=tot_saldo($idCli,$FechaC,$num_comp);

$saldoPeriodoAnt=$STATS_PAGOS["saldo"];
}else{
	
	
$STATS_PAGOS=tot_abon_cre($idCli);

$fechaUltimoPago="0000-00-00";
$valorUltimoPago="0";
$saldoPeriodoAnt=$STATS_PAGOS["saldo"] -$total;	
}
if($saldoPeriodoAnt==0){$saldoPeriodoAnt="";}


if($row['anulado']=="ANULADO"){$total="";}

// fin saldo ant
	
	if($tipoCli=="Mostrador")
	{
		$tot_mostrador+=$row['tot'];
		$tot_fac_mostrador++;
		
	}
	$NF=$row["nf"];
	
	
	if(1 ){
	?>
    <tr>

    <td><?php echo " $num_fac"; /*$pre*/ ?></td>
    <td colspan=""><?php echo $nomCli ?></td>
        <td colspan=""><?php echo $idCli ?></td>
        <td colspan=""><?php echo $direccion ?></td>
         <td colspan=""><?php echo $barrio ?></td>
        <td><?php echo "$plan" ?></td>
		<td><?php echo "$tipoPlan" ?></td>
		<td><?php echo "$estrato" ?></td>
        <td><?php echo money3($SUB19) ?></td>
    <td><?php echo money3($IVA19) ?></td>
     <?php if($fechaI<"2017-01-31"){?>   
    <td><?php echo money3($SUB16) ?></td>
    <td><?php echo money3($IVA16) ?></td>
   <?php }?>

   
    <td><?php echo money3($excento) ?></td>
    <td><?php echo money3($excluidos) ?></td>
    <td><?php echo money3($InteresMora) ?></td>
    <td><?php echo money3($valorAfiliacionExenta) ?></td>
    <td><?php echo money3($valorAfiliacionExcluida) ?></td>
    <td><?php echo money3($valorAfiliacionGravada) ?></td>
    <td><?php echo money3($saldoPeriodoAnt) ?></td>
    <td><?php echo money3($DESCUENTO) ?></td>
    <td><?php echo money3($total) ?></td>

    <td><?php echo $fecha ?></td>
    <td><?php echo $row['anulado'] ?></td>
    </tr>
    
    <?php
	}
	
}


?>
<tfoot>
<td></td><td></td><td></td><td></td><td></td><td></td>
<?php if($fechaI<"2017-01-31"){?><td></td><td></td><?php }?>
<td></td><td></td><td></td><td><?php echo money3( $TOTAL);?></td><td></td><td></td>
</tfoot>
</tbody>
    <?php
      
    ?>
    
    <style>
        #asd11 span{
            color:rgba(240,240,240,1);
            height:40px;
            font-size: 12px;
            font-weight: bold;
        }
        #asd11{
            background:rgba(100,100,100,1);
           /* display:none;*/
        }
    </style>
        

</table>

<div id="imp"  align="center">
 
 
</div> 
</div>
<?php include_once("FOOTER_UK.php"); ?>
<script language="javascript1.5" type="text/javascript" src="JS/jquery_browser.js"></script>
<script src="JS/bootstrap.min.js"></script>
<script src="JS/plugins/dataTables/datatables.min.js"></script>
<script language="javascript1.5" type="text/javascript">
$(document).ready(function(){
	var rangoFecha='<?php if(!empty($fechaI)){echo "$fechaI a $fechaF";}?>';
	$('.tablaDataTables').DataTable({
				/*columnDefs: [
        { type: 'numeric-comma', targets: 2 }],*/
			language: {url: 'locales/es.json'},
                dom: '<"html5buttons"B>lTfgitp',
                buttons: [
                    { extend: 'copy'},
                    {extend: 'csv'},
                    {extend: 'excelHtml5', title: '<?php echo "$NOMBRE_INFORME";?> '+rangoFecha, customize: function ( xlsx ){
                var sheet = xlsx.xl.worksheets['sheet1.xml'];
 
               
				
				// Loop over the cells in column `F`
                $('row c[r^="F"]', sheet).each( function () {
                    // Get the value and strip the non numeric characters
                    if ( $('is t', this).text().replace(/[^\d]/g, '') * 1 >= 500000 ) {
                        $(this).attr( 's', '20' );
                    }
                });
            }},
                    {extend: 'pdf', title: '<?php echo "$NOMBRE_INFORME ";?> '+rangoFecha ,footer:true,orientation:'landscape',pageSize:'letter',
					customize: function ( doc ) {
                // Splice the image in after the header, but before the table
				//doc.content[1].table.widths = [ '8%', '10%', '10%', '8%', '7%', '7%', '7%', '7%', '10%', '9%', '9%', '8%'];
				<?php if($fechaI<"2017-01-31")
				{echo "doc.content[1].table.widths = [ '8%', '10%', '10%', '8%', '7%', '7%', '7%', '7%', '7%', '7%', '10%', '9%', '9%', '8%', '8%'];";}
				else {echo "doc.content[1].table.widths = [ '8%', '8%', '8%', '8%', '10%', '10%', '7%', '7%', '10%', '10%', '10%', '8%', '8%'];";} ?> 
                doc.content.splice( 1, 0, {
                    margin: [ 0, 0, 0, 12 ],
                    alignment: 'center',
                    image: 'data:image/png;base64,'
                } );
                // Data URL generated by http://dataurl.net/#dataurlmaker
            }},

                    {extend: 'print',footer:true,
                     customize: function (win){
                            $(win.document.body).addClass('white-bg');
                            $(win.document.body).css('font-size', '10px');

                            $(win.document.body).find('table')
                                    .addClass('compact')
                                    .css('font-size', 'inherit');
                    }
                    }
                ]

            });
	
});
</script>
</body>
</html>