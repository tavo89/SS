...
$tabla="descuentos";
$col_id="id_pro";
$columns="id_cli,id_pro ref,fecha_mod,dcto,(select nombre from usuarios where id_usu='$id_cli') as nombre,(select detalle from productos where id_pro=ref) des,cod_su";
$url="dcto_cli.php";
...
$sql = "SELECT  $columns FROM descuentos WHERE cod_su=$codSuc AND id_cli='$id_cli' ORDER BY nombre LIMIT $offset, $limit"; 
...
$sql_busq="SELECT $columns FROM descuentos WHERE (id_pro LIKE '$busq%' OR des LIKE '$busq%')  AND id_cli='$id_cli'";
...
<h1>Descuentos por Producto - <?php echo "$nombre_cliente" ?></h1>
<div style="position:absolute; top:-4px; right:5px;"><img src="<?php echo $url_LOGO_A ?>" width="<?php echo $x ?>" height="<?php echo $y ?>"></div>


<div data-role="navbar">
<ul>
<li><a href="crear_descuento.php"  data-icon="grid" data-ajax="false">Crear Dcto. a Cliente</a></li>
...
<td><?php echo $ref; ?></td> 
<td><?php echo $des; ?></td>
<td align="center" id="D<?php echo $ii ?>" onDblClick="mod_dcto('<?php echo $id_cli ?>','<?php echo $ref ?>','<?php echo $dcto ?>',<?php echo $ii ?>)">
<?php echo $dcto; ?></td>
...
function mod_dcto(id_cli,id_pro,dcto,ii)
{
	var $td=$('#D'+ii);
	$td.html('');
	var html="<input type=\"text\" name=\"dcto\" value=\""+dcto+"\" id=\"txt_dcto"+ii+"\" onChange=\"update('descuentos','dcto',this.value,'id_cli','id_pro','"+id_cli+"','"+id_pro+"',"+ii+"); \" onBlur=\"clearx('"+ii+"','"+dcto+"')\">";
	$('#txt_dcto'+ii);
	var $box=$(html);
	$box.appendTo($td);
	$box.focus();	
