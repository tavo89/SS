...
$num_fac=$val;
$sql = "SELECT  $columns FROM fac_remi  WHERE  sede_destino=$codSuc $UserFilter AND tipo_cli='Traslado'   $A ORDER BY $order desc  LIMIT $offset, $limit"; 
...
////////////////////////////////////////////////////////////////// PAGINACION PARTE 2//////////////////////////////////////////////////// 
$sqlTotal = "SELECT COUNT(*) as total FROM fac_remi WHERE sede_destino=$codSuc $UserFilter AND tipo_cli='Traslado'   $A"; 
...
$sql_busq="SELECT $columns FROM fac_remi WHERE sede_destino=$codSuc  $UserFilter AND tipo_cli='Traslado' AND (num_fac_ven LIKE '$busq%' OR id_cli LIKE '$busq%' OR placa LIKE '$busq%' OR nom_cli LIKE '$busq%' OR vendedor LIKE '$busq%' OR prefijo LIKE '$busq%') $A";
...
//imp_fac($num_fac,$pre,$boton,2,$val2);
...
?>
<h1 align="center">MERCANC&Iacute;A RECIBIDA</h1>
...
<div class="uk-button-group" id="<?php echo "button_$ii"; ?>">
...
<li><a href="<?php echo $url ?>?opc=Imprimir&valor=<?php echo $cod_fac ?>&pre=<?php echo $row['prefijo'] ?>&tipo_imp=post&pag=<?php echo $pag ?>&valor2=<?php echo $codOrig ?>"><i class="uk-icon-print"></i>Imprimir</a></li>

<!--
<li><a href="#" onClick="retorna_tras('<?php echo $cod_fac ?>','<?php echo $row['prefijo'] ?>','<?php echo $codDest ?>','<?php echo $codOrig ?>');"><i class="uk-icon-mail-reply-all"></i>Retornar Excedente</a></li>
-->

<?php
if($estado!="ANULADO" && $confirma!="Recibido"){
?>
<li><a href="#" onMouseUp="confirma_tras('<?php echo $cod_fac ?>','<?php echo $row['prefijo'] ?>','<?php echo $codOrig ?>','<?php echo $codDest ?>')"><i class="uk-icon-check-square-o"></i> Confirma Tras.</a></li>
<?php
}
?>
...
?>
<li><a href="#" onMouseUp="anular_fac_remi('<?php echo $cod_fac; ?>','<?php echo $pre; ?>','<?php echo $codOrig; ?>');"><i class="uk-icon-remove"></i> Anular</a></li>
<?php 
}
?>
</ul>
...
function confirma_tras(numRemi,pre,orig,dest)
{
	var datos='num_remi='+numRemi+'&pre='+pre+'&sedeOrig='+orig+'&sedeDest='+dest;
	if(confirm("Ingresar Traslado?")){
	ajax_x('ajax/confirma_traslado.php',datos,function(resp){if(esVacio(resp)){simplePopUp("Traslado Recibido!");location.reload();}});
	}
};
function retorna_tras(nf,pre,dest,orig)
{
	var data='nf='+nf+'&pre='+pre+'&dest='+dest+'&orig='+orig;
	if(confirm('Desea retornar la Mercancia?'))
	{ajax_a('ajax/regresa_productos_a_principal.php',data,'Operacion Exitosa');}
};
