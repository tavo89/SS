...
$datosCliente = $Cliente->getDataCliente();
 //////////////////////////////////////////////////////////// FILTRO FECHA //////////////////////////////////////////////////////////////////////////
$fechaI="";
$fechaF="";
...
$tabla="servicio_internet_planes";
$codSuCol="cod_su";
$col_id="id";
// columnas para mostrar y modificar
//CONCAT(prefijo,' ' ,num_fac_ven) as $new_str = str_replace(' ', '', $old_str);

if($fac_servicios_mensuales==1){$columns=str_replace(' ', '',"id,nombre_servicio,nombreSucursal,subDominioApp,tipoPlan,precioplan,fechaCobro,ultimoPago,wasap");}
else{$columns=str_replace(' ', '',"id,estrato,anchobanda,precioplan,velbajada,velsubida,equipreco,servadic,tipo_cliente,habilitado");}

$hidColumns=str_replace(' ', '',"id");
$hidColumns=explode(",",$hidColumns);

$monedaColumns=str_replace(' ', '',"precioplan");
$monedaColumns=explode(",",$monedaColumns);

$fechaColumns=str_replace(' ', '',"fechaCobro,ultimoPago");
$fechaColumns=explode(",",$fechaColumns);

$selecColumns=str_replace(' ', '',"tipoPlan,nombre_servicio");
...
{
	$SelOpt1="<option value=\'ANUAL\'>ANUAL</option><option value=\'MENSUAL\'>MENSUAL</option>";
	$SelOpt2="<option value=\"ANUAL\">ANUAL</option><option value=\"MENSUAL\">MENSUAL</option>";
}
else{
	$SelOpt2="<option value=\"Residencial\">Residencial</option><option value=\"Empresarial\">Empresarial</option><option value=\"Comercial\">Comercial</option>";
}
$variablesComunes = new lib_CommonFormsElements();
$selectListaServiciosSubscripcion= $variablesComunes->getListaServiciosSuscripcion();

$selecOPTColumns=array("tipoPlan"=>$SelOpt1,
                       "nombre_servicio"=>$selectListaServiciosSubscripcion['listaJS']);
$selecOPTColumns2=array("tipoPlan"=>$SelOpt2,
                        "nombre_servicio"=>$selectListaServiciosSubscripcion['listaHtml']);

if($fac_servicios_mensuales==1){
	$modColumns=str_replace(' ', '',"nombre_servicio,nombreSucursal,subDominioApp,tipoPlan,precioplan,fechaCobro,ultimoPago,wasap");
}
else{
	$modColumns=str_replace(' ', '',"nombre_servicio,habilitado");
}
...
$url="servicios_gestion_planes_internet.php";
if($fac_servicios_mensuales==1){$ORDER_BY="ORDER BY nombre_servicio,precioplan ";}
else{$ORDER_BY="ORDER BY estrato ";}
...
$sql = "SELECT  $columns FROM $tabla WHERE  $codSuCol=$codSuc AND id_cli='$ID_CLIENTE' $ORDER_BY DESC    LIMIT $offset, $limit "; 
//echo $sql;
...
$sql_busq="SELECT $columns FROM $tabla WHERE $codSuCol=$codSuc  AND id_cli='$ID_CLIENTE' AND ( $ND    ) ";


$rs=$linkPDO->query($sql_busq);
	}
$sqlTotal = "SELECT COUNT(*) AS total FROM $tabla WHERE  $codSuCol=$codSuc  AND id_cli='$ID_CLIENTE'"; 
...
<li class=""><a href="clientes.php"  ><i class="uk-icon-list <?php echo $uikitIconSize ?>"></i>&nbsp;Clientes</a></li>
...
if($fac_servicios_mensuales==1){
	...
	// "id,nombreSucursal,subDominioApp,tipoPlan,precioplan,fechaCobro,ultimoPago,wasap"
	$columnas2="cod_su,id_cli,nombre_servicio,nombreSucursal,subDominioApp,tipoPlan,precioplan,fechaCobro,ultimoPago,wasap";
	$tipoCol=array(0=>"hidden","hidden",
	              "select","text","text","select","text","date","date","text" );
	$default=array(0=>"$codSuc","$ID_CLIENTE","",
	              "","","","","","","" );
}
else{
	$columnas2="cod_su,id_cli,estrato,anchobanda,precioplan,velbajada,velsubida,equipreco,servadic,tipo_cliente";
	$tipoCol=array(0=>"hidden","hidden","text","text","text","text","text","text","text","select" );
	$default=array(0=>"$codSuc","$ID_CLIENTE","","","","","","","" );
}

crear_any_form("$tabla",$columnas2,$tipoCol,$default, " function(){ return true;}", "successAny","Planes Internet $SEDES[$codSuc]","busq_planes();",$selecOPTColumns2);
?>
<h1 align="center">SERVICIOS DE SUBSCRIPCION </h1>
<pre align="center"> <?php echo "$datosCliente[nombre]<br>$ID_CLIENTE<br>$datosCliente[direccion]<br>$datosCliente[telefono]<br>$datosCliente[ciudad]";?> </pre>
...
function DEL_any(id)
{
	...
	ajax_x('ajax/FORMS/DEL_serv_internet.php',data,function(resp){
