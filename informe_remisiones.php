...
$sql="SELECT  nit,COUNT(num_fac_ven) as tot_fac,MAX(num_fac_ven) as ultima,MIN(num_fac_ven) as primera FROM fac_remi  WHERE nit=$codSuc AND DATE(fecha)>='$fechaI' AND DATE(fecha)<='$fechaF' AND tipo_fac!='cotizacion'  AND tipo_cli!='Traslado'";
...
<span align="center" style=" font-size:24px"><B>INFORME REMISIONES</B></span>
</p>
</td>
...
<?php
$filtroCerradas=" ";
//$sql="SELECT anulado,cod_contado, num_fac_ven, sub_tot, iva,(select SUM(sub_tot) from art_fac_remi where art_fac_remi.num_fac_ven=fac_remi.num_fac_ven and iva=0 and art_fac_remi.nit=fac_remi.nit) as excento,tot,vendedor, TIME(fecha) as hora, DATE(fecha) as fe, tipo_venta,tipo_cli FROM fac_remi, sucursal WHERE sucursal.cod_su =$codSuc AND fac_remi.nit = sucursal.cod_su AND (tipo_cli='Taller Honda' OR tipo_cli='Garantia FANALCA')   AND DATE(fecha)>='$fechaI' AND DATE(fecha)<='$fechaF' AND num_fac_ven NOT IN(SELECT num_fac_ven FROM fac_remi  WHERE  nit=$codSuc  AND DATE(fecha_anula)>='$fechaI' AND DATE(fecha_anula)<='$fechaF')";
//$filtro="and (clase='ROPA' OR clase='ROPAS')";

$excento_art="(select SUM(sub_tot) from art_fac_remi where art_fac_remi.num_fac_ven=fac_remi.num_fac_ven and iva=0 and art_fac_remi.nit=fac_remi.nit AND art_fac_remi.prefijo=fac_remi.prefijo  )";


$sub16_art="(select SUM(sub_tot/1.16) from art_fac_remi where art_fac_remi.num_fac_ven=fac_remi.num_fac_ven and iva=16 and art_fac_remi.nit=fac_remi.nit AND art_fac_remi.prefijo=fac_remi.prefijo )";


$sub05_art="(select SUM(sub_tot/1.05) from art_fac_remi where art_fac_remi.num_fac_ven=fac_remi.num_fac_ven and iva=5 and art_fac_remi.nit=fac_remi.nit AND art_fac_remi.prefijo=fac_remi.prefijo )";

$sub10_art="(select SUM(sub_tot/1.10) from art_fac_remi where art_fac_remi.num_fac_ven=fac_remi.num_fac_ven and iva=10 and art_fac_remi.nit=fac_remi.nit AND art_fac_remi.prefijo=fac_remi.prefijo )";

$sub19_art="(select SUM(sub_tot/1.19) from art_fac_remi where art_fac_remi.num_fac_ven=fac_remi.num_fac_ven and iva=19 and art_fac_remi.nit=fac_remi.nit AND art_fac_remi.prefijo=fac_remi.prefijo )";
...
$cols="anticipo_bono,id_cli,tipo_venta,prefijo as pre,nom_cli,anulado, num_fac_ven, sub_tot, iva,IFNULL(($excento_art),0) as excento,tot,vendedor, TIME(fecha) as hora, DATE(fecha) as fe, tipo_venta,tipo_cli,($sub16_art) as sub16,($sub05_art) as sub05,($sub10_art) as sub10,($sub19_art) as sub19";

//$sql="SELECT $cols FROM fac_remi WHERE  nit = $codSuc   AND DATE(fecha)>='$fechaI' AND DATE(fecha)<='$fechaF'  ORDER BY fecha";
$sql="SELECT $cols FROM fac_remi, sucursal WHERE sucursal.cod_su =$codSuc AND fac_remi.nit = sucursal.cod_su  AND DATE(fecha)>='$fechaI' AND DATE(fecha)<='$fechaF' $filtroCerradas ORDER BY fecha";
//echo "$sql";
$rs=$linkPDO->query($sql);
$tot_mostrador=0;
$tot_fac_mostrador=0;
...
while($row=$rs->fetch())
{
    ...
    <td><?php echo money3($SUB05) ?></td>
    <td><?php echo money3($IVA05) ?></td>
        ...
        { type: 'numeric-comma', targets: 2 }],*/
			language: {url: 'locales/es.json'},
                ...
                buttons: [
                    { extend: 'copy'},
                    {extend: 'csv'},
                    {extend: 'excelHtml5', title: 'REMISIONES '+rangoFecha, customize: function ( xlsx ){
                ...
                $('row c[r^="F"]', sheet).each( function () {
                    ...
                    {extend: 'pdf', title: 'REMISIONES '+rangoFecha ,orientation:'landscape',pageSize:'letter',footer:true,
