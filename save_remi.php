...
$sql="SELECT * FROM $TABLA WHERE num_fac_ven='$num_fac' AND prefijo='$PRE' AND nit='$codSuc'";
...
//$conex->query($sql)? null : $all_query_ok=false;
		...
		$sql="SELECT * FROM  $tabla2   WHERE nit='$codSuc' AND fecha_vencimiento='$feVenci' AND ref='$ref' AND cod_barras='$cod_bar'";
...
$sql="SELECT * FROM inv_inter WHERE id_pro='$ref' AND id_inter='$cod_bar'";
$stmt = $linkPDO->query($sql);
if ($row = $stmt->fetch()) {
...
//echo "$cantAnt != $cant && $uniAnt != $uni";
if($FLUJO_INV==1 && ($cantAnt!=$cant || $uniAnt!=$uni)){
	
$sql="UPDATE `inv_inter`  SET exist=(exist-($cant-$cantAnt )), unidades_frac=(unidades_frac-($uni-$uniAnt)) WHERE nit_scs='$codSuc' AND fecha_vencimiento='$feVenci' AND id_pro='$ref' AND id_inter='$cod_bar'";

//echo "ajusta INV [$sql]";

$validateQuery=$linkPDO->exec($sql);
if(!$validateQuery){$all_query_ok=false;echo "ERROR SQL: $sql <br>";}
