...
<span align="center"><B>Lista Facturas Anuladas</B></span>
</p>
</td>
...
<table align="center" width="100%" cellpadding="2" cellspacing="1" style="font-size:11px" rules="rows" id="TabClientes" class="display dataTable table table-striped table-bordered table-hover tablaDataTables">
<thead>
<tr style="font-size:12px;  font-weight:bold;">
<th width=\"90px\">#</th>
<th width=\"100\">No. Fac</th>
<th width=\"200\">Cliente</th>
<th width=\"200\">C.C. Cliente</th>
<th width=\"150\">Tipo Cli. </th>
<th width=\"150\">Forma Pago</th>
<th width=\"200\">Vendedor</th>
<th width=\"50\">TOT</th>
<th width=\"150\">Fecha Anulaci&oacute;n</th>
...
<?php
$ii=0;
$columns="num_fac_ven,id_cli,nom_cli,fecha,tel_cli,dir,mecanico,vendedor,sub_tot,iva,descuento,tot,placa,tipo_cli,anulado,prefijo,tipo_venta,fecha_anula";

$sql = "SELECT  $columns FROM fac_venta  WHERE  nit=$codSuc AND anulado='ANULADO' AND DATE(fecha_anula)>='$fechaI' AND DATE(fecha_anula)<='$fechaF' ORDER BY num_fac_ven desc";
...
while($row=$rs->fetch())
{
			...
			$fecha = $row["fecha"];
			$fechaAnula = $row["fecha_anula"];
			$tot = money_dt($row["tot"]*1);
			$placa = $row["placa"];
			$id_cli = $row["id_cli"];
...
<td style="font-size:11px;"><?php echo $id_cli ?></td> 
<td><?php echo $tipoCli ?></td>
<td><?php echo $formaPago ?></td>
<td style="font-size:11px;"><?php echo $vendedor ?></td>
<td><?php echo $tot ?></td>
<td style="font-size:11px;"><?php echo $fechaAnula ?></td>
<td style="font-size:11px;"><?php echo $fecha ?></td>
    </tr>
