...
$fechaI="";
$fechaF="";
$PAG_fechaI="fechaI_ven_dev";
$PAG_fechaF="fechaF_ven_dev";
...
if(isset($_SESSION['nom_cli_facVenDev'])){$nom_cli=limpiarcampo($_SESSION['nom_cli_facVenDev']);$C=" AND (nom_cli='$nom_cli' OR id_cli='$nom_cli')";
...
if(isset($_REQUEST['nom_cli']) && !empty($_REQUEST['nom_cli'])){$nom_cli=limpiarcampo($_REQUEST['nom_cli']); $_SESSION['nom_cli_facVenDev']=$nom_cli;$C=" AND (nom_cli='$nom_cli' OR id_cli='$nom_cli')";
$botonFiltroNom="<input class=\"uk-button uk-button-danger\" type=\"submit\" name=\"opc\" value=\"QUITAR NOMBRE\" data-inline=\"true\" data-mini=\"true\" >";
}
...
if($opc=="QUITAR NOMBRE")
{
	$botonFiltroNom="<input class=\"uk-button uk-button-success\" type=\"submit\" name=\"opc\" value=\"Buscar Nombre\" data-inline=\"true\" data-mini=\"true\" >";
	$nom_cli="";
	unset($_SESSION['nom_cli_facVenDev']);
	$C="";
}
...
if(isset($_REQUEST['filtroD'])){
	...
	$n = preg_split("[|]",$filtroD);
	if(empty($filtroD))$D="AND (resolucion='$n[0]' AND prefijo='$n[1]' AND fecha_resol='$n[2]' AND rango_resol='$n[3]')";
...
<th width=\"100\" align=\"center\">DIAN</th>
<th width=\"150\">Fecha</th>";


$tabla="fac_dev_venta";
$col_id="id_pro";
$columns="num_fac_ven,id_cli,nom_cli,fecha,tel_cli,dir,mecanico,vendedor,sub_tot,iva,descuento,tot,placa,tipo_cli,anulado,prefijo,tipo_venta,anticipo_bono,cod_caja,DATE(fecha) as fe, DATE(fecha_anula) as fe_anula,fecha_anula,estado_factura_elec";
$url="dev_ventas.php";
$url_dialog="dialog_invIni.php";
$url_mod="mod_fac_ven.php";
$url_new="fac_dev_ven.php";
if(val_secc($id_Usu,"fac_crea") || $rolLv==$Adminlvl)$url_new="fac_dev_ven.php";
else $url_new="#";
$order="fecha";
...
$num_fac=$val;
$sql = "SELECT  $columns FROM $tabla  WHERE  nit=$codSuc  $A   $C ORDER BY $order desc  LIMIT $offset, $limit"; 
...
$sql_busq="SELECT $columns FROM $tabla WHERE nit=$codSuc  AND (num_fac_ven = '$busq' OR id_cli = '$busq' OR placa LIKE '$busq%' OR nom_cli LIKE '$busq%') $A";
...
<div class="uk-width-9-10 uk-container-center">
		...
		<a class="uk-navbar-brand uk-visible-large" href="centro.php"><img src="Imagenes/logoICO.ico" class="icono_ss"> &nbsp;SmartSelling</a> 
			...
			<ul class="uk-navbar-nav uk-navbar-center" style="width:330px;">   <!-- !!!!!!!!!! AJUSTAR ANCHO PARA AGREGAR NUEVOS ELMENTOS !!!!!!!! -->
		
			
				<li class="ss-navbar-center"><a href="<?php echo $url_new ?>" ><i class="uk-icon-plus-square <?php echo $uikitIconSize ?>"></i>&nbsp;Devolucion</a></li>
...
imp_fac($num_fac,$pre,$boton,3,"no",0,''); 
...
?>
<h1 align="center">DEVOLUCIONES VENTAS</h1>
...
<!--
<table id="tabQuery" class="display dataTable table table-striped table-bordered table-hover">
<thead>
...
<table border="0" align="center" claslpadding="6px" bgcolor="#000000" class="uk-table uk-table-hover uk-table-striped"  id="tabQuery"> 
 <thead>
...
/*
$respuestaFuncion=strip_codigo("41577099983112378020000000935724390000002031879620160601");
$cupon=$respuestaFuncion[0];
$valor=$respuestaFuncion[1];
REEMPLAZE ESTA PARTE SI LOS QUIERE CONVERTIR A NUMERO, CON MULTIPLICARLOS X 1 BASTA
$cupon=$respuestaFuncion[0]*1;
$valor=$respuestaFuncion[1]*1;
echo "CUPON: $cupon, VALOR: $valor";
*/
...
$ii++;
			...
			$url_print="$url?opc=Imprimir&valor=$cod_fac&pre=$row[prefijo]&tipo_imp=post&pag=$pag";
...
<tr>
<td>
<a href="<?php echo $url_print  ?>" class="uk-icon-print uk-icon-button uk-icon-hover uk-icon-small">
</a>
</td>
<?php 
...
?>
<td>
<a href="#"  onMouseUp="anular_dev_venta('<?php echo $cod_fac; ?>','<?php echo $pre; ?>');" class="uk-icon-remove uk-icon-button uk-icon-hover uk-icon-small">
...
?>
<td>
<a href="#"  onMouseUp="SEND_facElec('<?php echo $cod_fac; ?>','<?php echo $pre; ?>','<?php echo $codSuc; ?>');" class="uk-icon-send  uk-icon-button uk-icon-hover uk-icon-small">
...
<tr>
<?php 
if($rolLv==$Adminlvl || (val_secc($id_Usu,"fac_mod")  &&  ($fe_anula==$fe ))){
?>
<!--
<td>
<a href="<?php echo $url_modificar; ?>" class="uk-icon-pencil uk-icon-button uk-icon-hover uk-icon-small" onClick="<?php echo $functMod; ?>">
...
<td><?php echo $tot ?></td>
<td <?php if($estado=="ANULADO"){echo "style=\"background-color:red; color:white; font-weigh:bold;\"";} ?>><b><?php echo $estado ?></b></td>
...
function SEND_facElec(num_fac,pre,cod_su)
{
	$.ajax({
		url:'ajax/FacElec/SEND_notaCre.php',
		data:{num_fac:num_fac,prefijo:pre,codSuc:cod_su},
		type:'POST',
		dataType:'JSON',
...
var query=getJson('ajax/query_ventas.php',{});	

$('#tabQuery').DataTable({data:query,
columns:[
...
dom: '<"html5buttons"B>lTfgitp',
 buttons: [
             {extend: 'copy'},
             {extend: 'csv'},
             {extend: 'excelHtml5', title: 'VENTAS (Utilidad x Producto) '+rangoFecha},
             {extend: 'pdf', title: 'VENTAS (Utilidad x Producto) '+rangoFecha ,orientation:'landscape',pageSize:'letter',footer:true,
		      customize: function ( doc ) {
                // Splice the image in after the header, but before the table
				//doc.content[1].table.widths = [ '10%', '10%', '15%', '5%', '15%', '15%', '6%', '15%'];
