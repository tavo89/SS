...
$rs=$linkPDO->query("SELECT * from fac_venta WHERE nit=$codSuc AND tipo_venta='Credito' AND ".VALIDACION_VENTA_VALIDA." AND estado!='PAGADO' AND tipo_cli='Empleados' GROUP BY id_cli order by 3");
$i=0;
while($row=$rs->fetch())
{
...
$sql="SELECT * from fac_venta WHERE nit=$codSuc AND tipo_venta='Credito' AND ".VALIDACION_VENTA_VALIDA."  AND tipo_cli='Empleados' GROUP BY num_fac_ven,prefijo";
...
<span align="center" style="font-size:24px"><B>CREDITOS POR COBRAR-EMPLEADOS</B></span>
</p>
</td>
...
for($i=0;$i<count($id_clientes);$i++)
{
	$tot_saldo=0;
	$tot_abono=0;
	$sql="SELECT *,DATEDIFF(CURRENT_DATE(),DATE(fecha) ) AS mora,DATEDIFF(DATE(fecha_pago),DATE(fecha) ) AS mora2 FROM fac_venta WHERE id_cli='".$id_clientes[$i]."' AND ".VALIDACION_VENTA_VALIDA." AND tipo_venta='Credito' AND estado!='PAGADO' AND tipo_cli='Empleados'";
	$rs=$linkPDO->query($sql);
	?>
<tr>
