<?php
include_once("Conexxx.php");
$url=thisURL();
$boton=r("boton");
$resumen=r("resumen");
$filtro=r("filtro");
$fechaMora=r("fecha_mora");
if($boton=="MS EXCEL"){excel("Cartera Clientes");}

$FILTRO_SEDES_FAC="AND nit=$codSuc";
$FILTRO_SEDES_COD_SU="AND cod_su=$codSuc";

if($MODULES["MULTISEDES_UNIFICADAS"]==1){$FILTRO_SEDES_FAC="";$FILTRO_SEDES_COD_SU="";}


//////////////////////////////////////////////////////////// FILTROS VARIOS //////////////////////////////////////////////////

$FILTRO_A="";
if($filtro==1){$FILTRO_A=" AND (DATEDIFF(CURRENT_DATE(),DATE(fecha) )>$DIAS_BAN_CLI AND fecha_pago='0000-00-00 00:00:00')";}

$FILTRO_MOROSOS_FECHA_X="";

$FILTRO_MORA_COMPROBANTES_WHERE=" ";
$FILTRO_MORA_COMPROBANTES=" ";
if(!empty($fechaMora)){$FILTRO_MOROSOS_FECHA_X=" AND (DATEDIFF('$fechaMora',DATE(fecha) )>=$DIAS_BAN_CLI AND (fecha_pago='0000-00-00 00:00:00' OR DATE(fecha_pago)>='$fechaMora' ))";

$FILTRO_MORA_COMPROBANTES_WHERE=" WHERE DATE(fecha)<='$fechaMora' ";
$FILTRO_MORA_COMPROBANTES=" AND  DATE(fecha)<='$fechaMora' ";
}



//////////////////////////////////////////////////////////// FILTRO FECHA ///////////////////////////////////////////////////
$fechaI="";
$fechaF="";
$PAG_fechaI="fechaI_cre";
$PAG_fechaF="fechaF_cre";
$A="";
$B="";

if(isset($_SESSION[$PAG_fechaI]) && !empty($_SESSION[$PAG_fechaI])){$fechaI=limpiarcampo($_SESSION[$PAG_fechaI]);}
if(isset($_SESSION[$PAG_fechaF]) && !empty($_SESSION[$PAG_fechaF])){$fechaF=limpiarcampo($_SESSION[$PAG_fechaF]);}

if(isset($_SESSION[$PAG_fechaF]) && !empty($_SESSION[$PAG_fechaF]) && isset($_SESSION[$PAG_fechaI]) && !empty($_SESSION[$PAG_fechaI]))
{
	$A=" AND (DATE(fecha)>='$fechaI' AND DATE(fecha)<='$fechaF') ";
}
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

/////////////////////////////////// FILTRO NOMBRE //////////////////////////////////////
$campo1="nom_cli";
$nom_cli="";
if(isset($_SESSION[$campo1]) && !empty($_SESSION[$campo1])){$nom_cli=$_SESSION[$campo1];$B=" AND (nom_cli='$nom_cli' OR id_cli='$nom_cli')";}


/////////////////////////////////////////////////////////////////////////////////////////


/////////////////////////////////////////////////////////////////////// FILTRO ESTADO FAC. ///////////////////////////////////////////////////////////

$filroEstado="";
if(isset($_SESSION['filtroB']))
{
	$filtroB=$_SESSION['filtroB'];
	if($filtroB=="Pendientes")$filroEstado="AND estado='PENDIENTE'";
	else if($filtroB=="Pagados")$filroEstado="AND estado='PAGADO'";
	else if($filtroB=="Morosos"){$filroEstado=" AND DATEDIFF(CURRENT_DATE(),DATE(fecha) )>$DIAS_BAN_CLI AND fecha_pago='0000-00-00 00:00:00' ";}
	else $filroEstado="";
	
}
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////



$trabajoTerceros=0;
$id_clientes[]="";
$nom_clientes[]="";
$abonos[][]=0;

$sql="SELECT id_cli,nom_cli,prefijo,num_fac_ven,snombr,apelli FROM 
	  fac_venta 
	  WHERE tipo_venta='Credito' $filroEstado $FILTRO_SEDES_FAC  AND ".VALIDACION_VENTA_VALIDA."  $A $B $FILTRO_A $FILTRO_MOROSOS_FECHA_X 
	  GROUP BY id_cli order by 3";
$rs=$linkPDO->query($sql);
$i=0;
while($row=$rs->fetch())
{
	$id_clientes[$i]=ucwords(strtolower($row["id_cli"]));
	$nombrCompleto="$row[nom_cli] $row[snombr] $row[apelli]";
	$nom_clientes[$i]=ucwords(strtolower($nombrCompleto));
	$abonos[$row['prefijo']][$row['num_fac_ven']]=0;
	$TOT_RETE[$row['id_cli']]=0;
	$i++;
	
}

$sql="SELECT prefijo,num_fac_ven from fac_venta WHERE  tipo_venta='Credito' $FILTRO_SEDES_FAC  AND ".VALIDACION_VENTA_VALIDA."  $A $B GROUP BY num_fac_ven,prefijo";
$rs=$linkPDO->query($sql);
while($row=$rs->fetch())
{
	$abonos[$row['prefijo']][$row['num_fac_ven']]=0;
	
	
}
 
$sql="SELECT SUM(a.abono) as tot,a.pre,a.num_fac,DATEDIFF(CURRENT_DATE(), MAX(DATE(fecha)) ) fe FROM cartera_mult_pago a INNER JOIN (SELECT cod_su,fecha,num_com,cajero,valor,num_fac,pre,anulado FROM comprobante_ingreso WHERE anulado!='ANULADO' $FILTRO_SEDES_COD_SU $FILTRO_MORA_COMPROBANTES ) b ON a.num_comp=b.num_com AND a.cod_su=b.cod_su WHERE  estado!='ANULADO'   $FILTRO_MORA_COMPROBANTES GROUP BY num_fac,pre";
$rs=$linkPDO->query($sql);

//echo "$sql<br><br><br><br><br><br><br>";

$DIAS_MORA_LAST_MES[][]=0;
while($row=$rs->fetch())
{
	$abonos[$row['pre']][$row['num_fac']]=$row['tot']*1;
	$DIAS_MORA_LAST_MES[$row['pre']][$row['num_fac']]=$row['fe']*1;
	
	
}


$sql="SELECT b.prefijo,b.num_fac,b.tot FROM fac_venta a 
INNER JOIN 
(SELECT prefijo,num_fac,SUM(valor) tot,a.cod_su,b.id_cli as idCli FROM comp_anti a INNER JOIN exp_anticipo b ON a.num_exp=b.num_exp AND a.cod_su=b.cod_su AND  a.cod_su='$codSuc' AND a.estado='COBRADO' GROUP BY prefijo,num_fac) b 
ON a.num_fac_ven=b.num_fac AND a.prefijo=b.prefijo 	AND a.id_cli=b.idCli AND a.nit=b.cod_su  $filroEstado AND ".VALIDACION_VENTA_VALIDA." AND tipo_venta='Credito' AND num_exp!=0  $A $B $FILTRO_A $FILTRO_SEDES_FAC ";
//echo "<div style=\"z-index:999\">$sql</div>";
$rs=$linkPDO->query($sql);
while($row=$rs->fetch())
{
	$abonos[$row['prefijo']][$row['num_fac']]+=$row['tot']*1;
	
	
}

$TOT_ANTICIPOS_SIN_COBRAR[]=0;

$sql="SELECT b.id_cli,prefijo,num_fac,SUM(valor) tot,a.cod_su FROM comp_anti a INNER JOIN exp_anticipo b ON a.num_exp=b.num_exp AND a.cod_su=b.cod_su AND  a.cod_su='$codSuc' AND b.estado='ABIERTO' GROUP BY id_cli";
$rs=$linkPDO->query($sql);
while($row=$rs->fetch())
{
	$TOT_ANTICIPOS_SIN_COBRAR[$row['id_cli']]=$row['tot']*1;
	//echo "<li>".$row['tot']."</li>";
	
}



$TOT_RETE[]=0;

$sql="SELECT IFNULL(SUM(r_fte+r_ica),0) AS t,id_cli,cod_su FROM comprobante_ingreso WHERE anulado!='ANULADO' $FILTRO_SEDES_COD_SU GROUP BY id_cli";
$rs=$linkPDO->query($sql);
while($row=$rs->fetch())
{
	$TOT_RETE[$row['id_cli']]=$row['t']*1;
	//echo "<li>".$row['t']."-".$row['id_cli']."</li>";
	
}

?>
<!DOCTYPE html PUBLIC  >
<html  >
<head>
<?php 
if($boton!="MS EXCEL"){
?>
<link href="font-awesome/css/font-awesome.css" rel="stylesheet">
<link href="css/animate.css" rel="stylesheet">
<link href="css/style.css" rel="stylesheet">
<link href="css/plugins/dataTables/datatables.min.css" rel="stylesheet">
<link href="css/bootstrap.min.css" rel="stylesheet">
 
 
<?php include_once("IMP_HEADER.php"); 
include_once("HEADER_UK.php"); 
}
?>

</head>

<body>
<div style=" top:0cm; width:23cm; height:27.9cm; position:relative; padding-left:15px;"  class=" ">
<?php //echo "<p style=\"z-index:999999\"><br>$sql</p>"; ?>


<table align="center" width="100%" >
<thead>
<tr>
<td colspan="3">
<?php echo $PUBLICIDAD2 ?></td>
<td valign="top" colspan="3">
<p align="left" style="font-size:12px;">
<span align="center" style="font-size:24px"><B>ESTADOS DE CUENTA-P&Uacute;BLICO</B></span>
</p>
</td>

</tr>
</table>
Fecha Informe: <?PHP echo $hoy ?>
<br>
<b>FILTROS</b>
<?php
if(!empty($A))echo "<li>FECHA:  Desde $fechaI Hasta $fechaF </li>";
if(!empty($B))echo "<li>CLIENTE:  $nom_cli </li>";
if(!empty($fechaMora))echo "<li>MOROSOS A <b>$fechaMora</b></li>";
if(!empty($filroEstado))echo "<li>Facturas <b>$filtroB</b></li>";

?>




<?php
$tot_saldo=0;
$tot_abono=0;
$TOT_ABONO=0;
$TOT_SALDO=0;
$SUM_RETE=0;
$TOT_DEV=0;
$dev_ventas=0;
$TOT_ASS=0;
$TOT_ASS_CLI=0;
$TOT_CARTERA=0;
for($i=0;$i<count($id_clientes);$i++)
{
	if(!empty($fechaF)){$dev_ventas=resta_dev_venta2($id_clientes[$i],$fechaF);}
	else { $dev_ventas=resta_dev_venta($id_clientes[$i]);}
	$tot_saldo=-$dev_ventas;
	$tot_abono=0;
	if(array_key_exists($id_clientes[$i],$TOT_RETE)){$SUM_RETE+=$TOT_RETE[$id_clientes[$i]];}
	
	if(array_key_exists($id_clientes[$i], $TOT_ANTICIPOS_SIN_COBRAR)){$TOT_ASS+= $TOT_ANTICIPOS_SIN_COBRAR[$id_clientes[$i]];}
	else{$TOT_ANTICIPOS_SIN_COBRAR[$id_clientes[$i]]=0;}
	
	$TOT_DEV+=$dev_ventas;
	$sql_mora="DATEDIFF(CURRENT_DATE(),DATE(fecha) )";
	//$sql_mora="DATEDIFF(CURRENT_DATE(),DATE(SELECT fecha FROM ) )";
	$sql_mora2="DATEDIFF(DATE(fecha_pago),DATE(fecha) )";
	if(!empty($fechaMora)){
	$sql_mora="DATEDIFF('$fechaMora',DATE(fecha) )";
	$sql_mora2=$sql_mora;
	}
	$abonoSqry="(tot-r_fte-r_ica-r_iva)";
	//$abonoSqry="(tot)";
	$sql="SELECT *,DATE(fecha) as fe,$sql_mora AS mora,$sql_mora2 AS mora2,fecha_pago,$abonoSqry as TOT FROM fac_venta WHERE id_cli='$id_clientes[$i]' $filroEstado AND ".VALIDACION_VENTA_VALIDA." AND tipo_venta='Credito'  $A $B $FILTRO_A $FILTRO_SEDES_FAC $FILTRO_MOROSOS_FECHA_X";
	//echo "$sql";
	$rs=$linkPDO->query($sql);
	if($resumen!=1){
	?>
<table align="center"     class="uk-table uk-table-condensed">
<thead>
<tr>
<?php if($resumen!=1){?>
<td width="80px">Fecha</td><td width="80px">Factura</td><td width="150px" align="right">Valor Fac.</td><td width="150px" align="right">Abonado</td><td width="150px" align="right">Saldo</td><td width="" align="right">Ult. Pago</td>
<?php }else{?>
<!--<th   width="80px">Nombre Cliente</th><th width="150px">Abonado</th><th width="150px">Saldo</th> -->
<?php }?>
</tr>
</thead>
 
<tbody>  


<tr>
<td colspan="9" class="uk-block-primary"><b><?php echo $id_clientes[$i]." - ".$nom_clientes[$i] ?></b></td>
</tr> 
    <?php
	}
	while($row=$rs->fetch())
	{
		$fechaPago=$row["fecha_pago"];
		$pre=$row['prefijo'];
		$numFac=$row['num_fac_ven'];
		$sede=$row['nit'];
		$totFac=$row['TOT']*1;
		$fecha=$row['fe'];
		if(array_key_exists($pre,$DIAS_MORA_LAST_MES)  && array_key_exists($numFac,$DIAS_MORA_LAST_MES) ){$mora=$DIAS_MORA_LAST_MES[$pre][$numFac];}
		else {$mora=$row['mora'];}
			$mora2=$row['mora2'];
			$estado = $row["estado"];
			if($estado=="PAGADO" && empty($fechaMora)){$mora=$mora2; }
			$saldo=$totFac-$abonos[$pre][$numFac];
		$abono=$abonos[$pre][$numFac];
		$tot_abono+=$abono;
		$tot_saldo+=$saldo;
		$TOT_ABONO+=$abono;
		$TOT_SALDO+=$saldo;
		$TOT_CARTERA+=$totFac;
		
		if($saldo<=0){$mora='<div class="uk-badge uk-badge-success">PAGADO</div>';}
		
		fix_cartera($id_clientes[$i],$numFac,$pre,$codSuc,$abono,$totFac,$estado,$fechaPago);
		if(!empty($B)){fix_cartera($id_clientes[$i],$numFac,$pre,$codSuc,$abono,$totFac,$estado,$fechaPago);}
		if($resumen!=1){
		?>
 <tr>
 <td align="left" width="120px"><?php echo $fecha ?></td>
 <td align="left" width="120px" style="font-size:18px; font-weight:bold;"><?php echo "$pre $numFac" ?></td>
 
 <td align="right"><?php echo money3($totFac) ?></td>
 <td align="right"><?php echo money3($abonos[$pre][$numFac]) ?></td>
 <td align="right"><?php echo money3($totFac-$abonos[$pre][$numFac]) ?></td>
 <td align="right" width="60px" colspan=""><?php echo $mora ?></td>
 
 </tr>
<?php
}
if($abonos[$pre][$numFac]>0 && !empty($B)){echo "<tr><td colspan=\"6\"><b>Pagos:</b></td></tr>";

$sqlAbonos="SELECT b.cajero, b.num_com,b.valor,a.abono,DATE(b.fecha) as fecha2 FROM cartera_mult_pago a LEFT JOIN (SELECT fecha,cod_su,num_com,cajero,valor,num_fac,pre,anulado FROM comprobante_ingreso WHERE cod_su='$codSuc' $FILTRO_MORA_COMPROBANTES ) b ON a.num_comp=b.num_com WHERE ((a.num_fac='$numFac' AND a.pre='$pre') OR (b.num_fac='$numFac' AND b.pre='$pre')) AND b.anulado!='ANULADO' AND a.cod_su=b.cod_su AND a.cod_su='$codSuc' ORDER BY b.fecha";
$rsAbonos=$linkPDO->query($sqlAbonos);
?>

<tr>
<td colspan="6">

<table align="center"          class="uk-table uk-table-condensed">
<?php
while($rowAbonos=$rsAbonos->fetch())
{
	
$numCompIngreso=$rowAbonos['num_com'];
$fechaComp=$rowAbonos['fecha2'];
$valorComp=$rowAbonos['abono'];
$cajeroComp=$rowAbonos['cajero'];
	?>
<tr>
<td><?php echo "$fechaComp"; ?></td>
<td colspan="2">Comprobante <?php echo "No. $numCompIngreso"; ?></td>

<td align="right"><?php echo money3("$valorComp"); ?></td>
</tr>
    
    
    <?php
	
}
?>
</table>
 </td>
    </tr>
<tr>
<td colspan="6" style="font-size:18px; font-weight:bold" align="center"><div style="border-bottom-style:dotted;">FIN FACTURA <?php echo  "$pre $numFac "; ?></div></td>
</tr>
<?php
	}// FIN ABONOS DISPONIBLES
	}//FIN WHILE MAIN
	?>


<tr style="<?php if($resumen!=1){echo "font-weight:bold;";} ?>" class=" ">
<td  align="left" colspan="4"> <?php echo strtoupper($nom_clientes[$i]) ?></td>
<td align="right" width="100px" class="<?php  if($MODULES["RETENCIONES"]!=1){echo "uk-hidden";}?>">R. (ICA / FTE)</td>
<td align="right" width="250px">ABONOS</td>
<td align="right" width="100px" class="<?php  if($MODULES["cartera_restar_devoluciones"]!=1){echo "uk-hidden";}?>">DEVOLUCIONES</td>
<td align="right" width="250px" colspan="">SALDO</td>
</tr>    
<!--
 <TR>
    
 <td  align="left">  </td><td></td><td><b>ANTICIPOS</b></td><td align="right" width="250px"><?php echo money3($TOT_ANTICIPOS_SIN_COBRAR[$id_clientes[$i]]) ?></td><td align="right" width="100px"></td><td align="right" width=""></td>
 
    </TR>
   --> 
<tr style="<?php if($resumen!=1){echo "background-color: #CFF";} ?>">
<td  align="left"></td>
<td></td>
<td></td>
<td></td>
<td align="right" class="<?php  if($MODULES["RETENCIONES"]!=1){echo "uk-hidden";}?>"><?php echo money3($TOT_RETE[$id_clientes[$i]]) ?></td>
<td align="right"><?php echo money3($tot_abono) ?></td>
<td align="right" class=" <?php  if($MODULES["cartera_restar_devoluciones"]!=1){echo "uk-hidden";}?>"><?php  echo money3($dev_ventas)?></td>
<td align="right" colspan=""><?php   if($tot_saldo>0){echo '<div class="uk-badge uk-badge-warning" style="font-size:16px;">'.money3($tot_saldo).'</div>';}else {echo '<div class="uk-badge uk-badge-success">PAGADO</div>';}?></td>
</tr> 




</tbody>
</table>
<br><br>

<?php
}

$TOT_SALDO-=$TOT_DEV;
?>


<br><br>
 
 


<div class="uk-grid">
    <div class="uk-width-medium-1-1">
        <div class="uk-panel uk-panel-box-primary">
		<h3 class=" uk-badge-success" style="font-size:14px;" align="left"> PAGOS </h3>
		<h1 class="uk-heading-medium" style="color:black;"><?php echo money3($TOT_ABONO) ?></h1>
        <br>
        </div>
    </div>
     
    <div class="uk-width-medium-1-1">
        <div class="uk-panel uk-panel-box-primary">
		<h3 class="uk-badge-warning" style="font-size:14px;"> SALDO </h3>
		<h1 class="uk-heading-medium" style="color:black;"><?php  echo money3($TOT_SALDO)?></h1>
        <br>
        </div>
    </div>
    
    <div class="uk-width-medium-1-1" style="border:1px;">
        <div class="uk-panel uk-panel-box-primary">
		<h3 class="uk-badge-success" style="font-size:14px;"> TOTAL CARTERA </h3>
		<h1 class="uk-heading-medium" style="color:black;" align="left"><?php echo money3($TOT_CARTERA) ?></h1>
        </div>
        
    </div>
</div>

<div id="imp" align="center">

<input type="button" value="IMPRIMIR" name="boton" onClick="imprimir()" />
<input type="button" value="MS EXCEL" name="boton" onClick="location.assign('<?php if($resumen!=1 && $filtro!=1 && empty($fechaMora)){echo "$url?boton=MS EXCEL";}else {echo "$url&boton=MS EXCEL";} ?>')" />


</div>
</div>

<?php 
if($boton!="MS EXCEL"){
include_once("FOOTER_UK.php"); 
?>

<script language="javascript1.5" type="text/javascript" src="JS/jquery_browser.js"></script>
<script src="JS/bootstrap.min.js"></script>
<script src="JS/plugins/dataTables/datatables.min.js"></script>
<script src="JS/plugins/dataTables/numericCommaPlugIn.js"></script>
<script language="javascript1.5" type="text/javascript">
$(document).ready(function(){
	var rangoFecha='<?php if(!empty($fechaI)){echo "$fechaI a $fechaF";}?>';
	$('.tablaDataTables').DataTable({
				/*columnDefs: [
        { type: 'numeric-comma', targets: 2 }],*/
				language: {url: 'locales/es.json'},
                dom: '<"html5buttons"B>lTfgitp',
                buttons: [
                    { extend: 'copy'},
                    {extend: 'csv'},
                    {extend: 'excelHtml5', title: 'Rotacion Inventario', customize: function ( xlsx ){
                var sheet = xlsx.xl.worksheets['sheet1.xml'];
 
                // jQuery selector to add a border
                $('row c[r*="10"]', sheet).attr( 's', '25' );
				
				
				// Loop over the cells in column `F`
                $('row c[r^="F"]', sheet).each( function () {
                    // Get the value and strip the non numeric characters
                    if ( $('is t', this).text().replace(/[^\d]/g, '') * 1 >= 500000 ) {
                        $(this).attr( 's', '20' );
                    }
                });
            }},
                    {extend: 'pdf', title: 'Rotacion Inventario' ,pageSize:'letter',footer:true,orientation:'portrait',
					customize: function ( doc ) {
                // Splice the image in after the header, but before the table
				//doc.content[1].table.widths = [ '10%', '10%', '15%', '12%', '5%', '5%', '5%', '5%', '5%', '12%','*'];
				
                doc.content.splice( 1, 0, {
                    margin: [ 0, 0, 0, 12 ],
                    alignment: 'center',
                    image: 'data:image/png;base64,'
                } );
                // Data URL generated by http://dataurl.net/#dataurlmaker
            }},

                    {extend: 'print',
                     customize: function (win){
                            $(win.document.body).addClass('white-bg');
                            $(win.document.body).css('font-size', '10px');

                            $(win.document.body).find('table')
                                    .addClass('compact')
                                    .css('font-size', 'inherit');
                    }
                    }
                ]

            });
	
});
</script>

<?php 
}
?>
</body>
</html>