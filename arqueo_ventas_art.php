...
$sql="SELECT cod_barras,nom_cli,art_fac_ven.num_fac_ven, AVG(precio) precio,des,art_fac_ven.sub_tot,art_fac_ven.iva,cant,unidades_fraccion,fraccion,ref, TIME(fecha) as hora, DATE(fecha) as fe, tipo_venta,tipo_cli,vendedor,fac_venta.prefijo,art_fac_ven.prefijo pre,cod_caja FROM fac_venta INNER JOIN art_fac_ven ON fac_venta.num_fac_ven=art_fac_ven.num_fac_ven WHERE fac_venta.prefijo=art_fac_ven.prefijo AND fac_venta.nit=art_fac_ven.nit AND art_fac_ven.nit=$codSuc $filtroCaja    AND (DATE(fecha)>='$fechaI' AND DATE(fecha)<='$fechaF' $filtroHora) $filtroNOanuladas GROUP BY ref,cod_barras,fecha_vencimiento,num_fac_ven";
//echo "$sql";
...
while($row=$rs->fetch())
{
	...
	$des=$row['des'];
	$cant=$row['cant']*1;
	$unidades_fraccion=$row["unidades_fraccion"];
	$fraccion=$row["fraccion"];
	if($fraccion==0){$fraccion=1;}
	$factor=(($unidades_fraccion/$fraccion)+$cant);
	$totalReferenciasVendidas+=$factor;
    ...
    <td align="center"><?php echo "$cant;$unidades_fraccion" ?></td>
...
<td></td>
<td></td>
<td><?php echo "$totalReferenciasVendidas";?></td> 
<td></td>
<td></td>
<td><?PHP echo money_dt(redondeo($TOT_VENTAS0516[1][1])) ?></td>
</tr>
...
$tituloInforme="Ventas Productos";
?>
