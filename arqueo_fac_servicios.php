...
<title>FACTURAS VENTAS SERVICIOS</title>
...
<span align="center" style=" font-size:24px"><B>FACTURAS VENTAS SERVICIOS</B></span>
</p>
</td>
...
$cols="anticipo_bono,id_cli,tipo_venta,prefijo as pre,nom_cli,snombr,apelli,anulado, num_fac_ven, sub_tot, iva,IFNULL(($excento_art),0) as excento,tot,vendedor, TIME(fecha) as hora, DATE(fecha) as fe, tipo_venta,tipo_cli,($sub16_art) as sub16,($sub05_art) as sub05,($sub10_art) as sub10,($sub19_art) as sub19";

//$sql="SELECT $cols FROM fac_venta WHERE  nit = $codSuc   AND DATE(fecha)>='$fechaI' AND DATE(fecha)<='$fechaF'  ORDER BY fecha";
$sql="SELECT $cols FROM fac_venta, sucursal WHERE sucursal.cod_su =$codSuc AND fac_venta.nit = sucursal.cod_su  AND DATE(fecha)>='$fechaI' AND DATE(fecha)<='$fechaF' $filtroCerradas ORDER BY fecha";
//echo "$sql";
$rs=$linkPDO->query($sql);
$tot_mostrador=0;
$tot_fac_mostrador=0;
...
while($row=$rs->fetch())
{
	...
	$SUB19=round($SERV[19]);
	$IVA19=round(($SERV[19])*0.19);
	
	$SUB10=round($SERV[10]);
	$IVA10=round(($SERV[10])*0.10);
	
	$SUB16=round($SERV[16]);
	$IVA16=round(($SERV[16])*0.16);
	$SUB05=round($SERV[5]);
	$IVA05=round(($SERV[5])*0.05);
	$excento=$SERV[0];
        ...
        { type: 'numeric-comma', targets: 2 }],*/
			language: {url: 'locales/es.json'},
                ...
                buttons: [
                    { extend: 'copy'},
                    {extend: 'csv'},
                    {extend: 'excelHtml5', title: 'VENTAS (Utilidad x Producto) '+rangoFecha, customize: function ( xlsx ){
                ...
                $('row c[r^="F"]', sheet).each( function () {
                    ...
                    {extend: 'pdf', title: 'FACTURAS VENTAS SERVICIOS '+rangoFecha ,orientation:'landscape',pageSize:'letter',footer:true,
