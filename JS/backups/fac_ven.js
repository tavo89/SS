...
var ifB2=frm.ced.value.trim()=='222222222' &&(frm.cli.value.trim()!=CLIENTE_MOSTRADOR);
var ifB3=frm.ced.value.trim()!='222222222' &&( frm.cli.value.trim()==CLIENTE_MOSTRADOR);
...
{
	//console.log('gfv_block, flag_gfv:'+flag_gfv);
...
//alert(cambio);
   ...
   else if(topeCre<(totFacCre+totFac) && frm.ced.value!="222222222" && topeCre!=0){warrn_pop('Esta factura supera el limite de Credito por el sig. monto:[$'+puntob(excesoCredito)+']');}
   else if( (frm.ced.value=="222222222" || frm.cli.value=="CLIENTE GENERAL") && frm.form_pago.value=="Credito"){warrn_pop("Cliente general no tiene autorizado creditos");}
   else  if(val_fv(frm)){}
