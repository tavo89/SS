...
{
var original=parseFloat(numero);
var result=Math.round(original*0.001)/0.001;
//var result=Math.round(original);
return result;
};
function puntoiva(n) {
	...
	for(i=0;i<=cont;i++)
	{
		...
		if($('#val_t'+i).length!=0)
		{
			...
			cant=val_ele('#'+i+'cant_')*1||0;
  			...
  			LimCant=redondeox(val_ele('#'+i+'cant_L'),2);
   			factor=(uni/frac)+cant;
			if(factor==0)factor=cant;
			factor=redondeox(factor,2);
...
function tot()
{
	...
	var SUBi=0,DCTOi=0,IVAi=0,TOTi=0,EXC=0,i=0,vi=0,vt=0,valOrig=0,uni=0,frac=0,factor=0;
	var $vst='';
	var Tcant=0;
	...
	SUBi=SUBi;
	TOTi=SUBi+IVAi+impuestoBolsas;
	//alert(TOTi+', '+puntob(redondeox(TOTi,2)));
	
	if(dcto_remi==28){
	$sub.prop('value',puntob(redondeox(SUBi,2)));
	$dcto.prop('value',puntob(DCTOi));
	$iva.prop('value',puntob(redondeox(IVAi,2)));
	$tot.prop('value',puntob(redondeox(TOTi,2)));
	...
	$sub.prop('value',puntob(redondeox(SUBi,2)));
	$dcto.prop('value',puntob(redondeox(DCTOi,2)));
	$iva.prop('value',puntob(redondeox(IVAi,2)));
	$tot.prop('value',puntob(redondeox(TOTi,2)));
	$exc.prop('value',puntob(EXC));
	/*
...
puntoa(n);
var stot=quitap($('#SUB').val())*1,tot=quitap($('#TOTAL').val())*1,r_fte=quitap($('#R_FTE').val())*1||0,r_iva=quitap($('#R_IVA').val())*1||0,r_ica=quitap($('#R_ICA').val())*1||0;
var impoConsumo=0;
if(impuesto_consumo==1)impoConsumo= redondeo(stot*IMPUESTO_CONSUMO);
...
function dct(id)
{
   ...
   var vuf=redondeo2(vsi - vsi2*dcto);
	  ...
	  $.ajax({
		...
		success:function(text){//alert(text);
			if(text!=0){
				...
				var dc=rs[0];
				//alert('tipo:'+tipo+'/////'+dc);
				if(tipo=="PRODUCTO"){dc=d(dc);}
				$dcto.prop('value',dc).focus().blur();
				$dcto_cli.prop('value',rs[0]);
				$tipo_dcto.prop('value',tipo);
...
function valor_t(id) {
  ...
  // alert('ID:'+id+', num:'+i);
   ...
   var uni=val_ele('#unidades'+i)*1||0;
   var frac=val_ele('#fracc'+i)*1||1;
   var LimCant=redondeox(val_ele('#'+i+'cant_L'),2);
   var dcto=(val_ele('#dcto_'+i)/100)||0;
   var iva=(val_ele('#iva_'+i)/100 + 1)||0;
   var factor=(uni/frac)+cant;
   if(factor==0)factor=cant;
   
   var factor2=redondeox(factor,2);
 ...
 /*
  ...
  if(factor2>LimCant&&cant!=1)
  {
   ...
   if(tipoRedondeo=="decimal")$vst.prop('value',puntob(redondeox(vt,2)) );
   else $vst.prop('value',puntob(redondeo2(vt)) );
...
contTO=0;
 ...
 if(cont>=40000 && modFac==1){
	...
	if(confirm('Existen otras Ref. con Fecha de Vencimiento diferente, Desea buscar otras Referencias??'))
			   ...
			   //descripcion
			   html+='<td class="art'+cont+'"><textarea style=" width:250px" cols="10" rows="1" class="art'+cont+'" name="det_'+cont+'" id="det_'+cont+'" readonly="">'+r[2]+'</textarea></td>';
			   
			   
			   //ubicacion - presen
			   if(usa_ubica==1){html+='<td class="art'+cont+'"><input class="uk-form-small art'+cont+'"  value="'+r[22]+'" type="text" id="ubica'+cont+'" name="ubica'+cont+'" placeholder="" style=" width:100px" /><input class="uk-form-small art'+cont+'"  value="'+r[17]+'" type="hidden" id="presentacion'+cont+'" name="presentacion'+cont+'" placeholder="" style=" width:100px" readonly/></td>';}
			  ...
			  if(usarTalla==1)html+='<td class="art'+cont+'" align="center"><input class="uk-form-small art'+cont+'"  value="'+r[9]+'" type="text" id="talla'+cont+'" name="talla'+cont+'" placeholder="" style=" width:40px" readonly/></td>';
			   ...
			   if(kardex==1)html+='<td class="art'+cont+'" align="center"><input class="uk-form-small art'+cont+'" id="iva_'+cont+'" type="text" name="iva_'+cont+'" size="5" maxlength="5" value=\"'+r[3]+'\" onkeyup="javascript:valor_t('+cont+');" style=" width:50px" readonly=""/></td>';
...
//move($('#'+cont+'cant_'),$('#'+(cont-1)+'cant_'),$('#'+(cont+1)+'cant_'),$('#'+cont+'cant_'),$('#unidades'+cont));
			  ...
			  // $cod.prop('value',r[21]);  
			   ...
			   //descripcion
			   if(vendeSin==0 || r[1]!='VENTA-000')html+='<td class="art'+cont+'"><textarea style=" width:250px" cols="10" rows="1" class="art'+cont+'" name="det_'+cont+'" id="det_'+cont+'" readonly="">'+r[2]+'</textarea></td>';
...
contTO=0;
 ...
 if(cont>=40000 && modFac==1){
	 ...
	 $('#butt_gfv').click();
		...
		data:{codBar:$cod=$('#cod').val(),fe_ven:$('#feVen').val(),ref:$('#Ref').val(),tc:tipoCli,idCli:''+$('#ced').val()+'',co:cotiza},
	    type: 'POST',
		dataType:'text',
		success:function(text){
		var resp=text;
		...
		//alert('text:['+text+']; resp:'+resp);
		    ...
		    if(resp==0) {
			 ...
			 $('#cod').blur();
			   ...
			   //valor unitario/val Min
			   html+='<td class="art'+cont+'" ><input class="uk-form-small art'+cont+'" id="val_u'+cont+'" name="val_uni'+cont+'" type="text" onkeyup="puntoa($(this));keepVal('+cont+');valor_t('+cont+');" value=\"'+puntob(parseFloat(vsi))+'\"  onBlur="valMin($(this),'+r[6]+','+vsi+','+cont+');change16('+cont+');"/><input class="uk-form-small art'+cont+'" id="val_u'+cont+'HH" name="val_u'+cont+'" type="hidden"  value="0" /><input class="uk-form-small art'+cont+'" id="val_u'+cont+'H" name="val_u'+cont+'H" type="hidden" readonly="" value="0" /><input class="uk-form-small art'+cont+'" id="valMin'+cont+'" type="hidden" name="valMin'+cont+'" size="5" maxlength="5" value="'+r[6]+'" style=" width:30px"/><input class="uk-form-small art'+cont+'" id="val_orig'+cont+'" name="val_orig'+cont+'" type="hidden"  value=\"'+puntob(parseFloat(vsi))+'\" /><input class="uk-form-small art'+cont+'" id="val_origb'+cont+'" name="val_origb'+cont+'" type="hidden"  value=\"'+parseFloat(r[4])+'\" /></td>';
			   
			   html+='<td class="art'+cont+'"><input class="uk-form-small art'+cont+'" id="val_t'+cont+'" name="val_tot'+cont+'" type="text" readonly="" value="0" /><input class="uk-form-small art'+cont+'" id="val_t'+cont+'HH" name="val_t'+cont+'" type="hidden" readonly="" value="0" /><input class="uk-form-small art'+cont+'" id="val_t'+cont+'H" name="val_t'+cont+'H" type="hidden" readonly="" value="0" /></td>';
	...
	if($('#remision').lenght!=0){tipoDoc=$('#remision').val();}
		...
		success:function(text){
			...
			if(resp!=0)
			{
			  ...
			  //alert(campos+' camión');
			  campos=campos.split('|');
			 $('#cli').prop('value',campos[0]);	
			 $('#dir').prop('value',campos[1]);
			 $('#tel').prop('value',campos[2]);
			 $('#city').prop('value',campos[3]);
			 $('#mail').prop('value',campos[4]);
			 $('#fe_naci').prop('value',campos[5]);
			 $('#ced').prop('value',campos[6]);
			 
			 $('#aliasCli').prop('value',campos[11]);	
			 ...
			 if(campos[10]=="Otros Talleres"){
				 alert('Otros Taller!');
				 $("#tipo_cli option[value='Otros Talleres']").prop('selected','selected');
			 }
			 
			 if(campos[7]==0 && tipoDoc!=3){
				 alert('El cliente '+campos[0]+' NO tiene autorizacion para creditos');
			 ...
			 if(campos[8]<campos[9] && campos[8]!=0 && tipoDoc!=3){alert('El cliente '+campos[0]+' supera el limite de credito establecido ('+puntob(campos[8])+')');
			 $("#form_pago option[value='Credito']").remove();
			 addCre=0;
			 }
			 else {
				 ...
				 if(addCre==0 && !(campos[8]<campos[9] && campos[8]!=0) && (campos[7]!=0) && tipoDoc!=3){
...
// $n.bind('keyup',function(e){
	...
	//alert('key:'+key);
	if(key==13&&(op=='add'||op=='dev')){add_art_ven();}
	if(key==13&&(op=='dev_ven')){add_art_dev_ven();}
	if(key==13&&op=='mod'){add_art_ven_mod();}
	if(key==13&&op=='mod_remi'){add_art_remi_mod();}
	if(key==120&&op!='dev'){busq($('#cod'));}
	if(key==120&&op=='dev'){busq($('#cod'));}
	if(key==120&&op=='dev_ven'){busq_dev($('#cod'));}
	//});
...
var ifB2=frm.ced.value.trim()=='222222222' &&(frm.cli.value.trim()!='PUBLICO GENERAL' && frm.cli.value.trim()!='CLIENTE GENERAL');
var ifB3=frm.ced.value.trim()!='222222222' &&(frm.cli.value.trim()=='PUBLICO GENERAL' || frm.cli.value.trim()=='CLIENTE GENERAL');
...
else if(1 && ( ifB1 || ifB2  || ifB3 ) )
	{warrn_pop('Registre el Cliente') ;
	if(esVacio(frm.cli.value))focusRed($('#cli'));
	else if(esVacio(frm.ced.value))focusRed($('#ced'));
	return true;
	}
	else if( (esVacio(frm.exi_ref.value)|| frm.exi_ref.value<=0) && valRemi){warrn_pop('Debe cargar articulos a la Factura!');$('#cod').focus();return true;}
	//else if(1){}
	else return false;
};
...
//alert(cambio);
   ...
   if(val_exist()){alert('Hay cantidades que superan las permitidas!!');}
	...
	if(frm.confirm_bono.value!='SI'){alert('Especifique el uso de Bono/Anticipo');focusRed($(frm.confirm_bono));}
					else {
						$('#'+id).prop("value",val);
    					$btn.off( 'click' );
...
function envia($input)
{
	...
	.keydown(function(e) {
		...
		switch(e.keyCode) {
			...
			e.preventDefault();
				//alert('enter!');
				gfv($('#butt_gfv'),'genera',document.forms['form_fac']);
...
//	var colorPK=$('#color'+c).val();
	...
	$.ajax({
		...
		success:function(text){
			...
			if(!esVacio(text)){alert('RESP saveFV:'+text);}
...
function addRow(r,Fname,EliF)
{
	...
	vsi=redondeo(vsi);
			   ...
			   //presentacion
			   html+='<td class="art'+cont+'"><input class="uk-form-small art'+cont+'"  value="'+r[17]+'" type="text" id="presentacion'+cont+'" name="presentacion'+cont+'" placeholder="" style=" width:100px" readonly/></td>';
			  ...
			  if(usarTalla==1)html+='<td class="art'+cont+'" align="center"><input class="uk-form-small art'+cont+'"  value="'+r[9]+'" type="text" id="talla'+cont+'" name="talla'+cont+'" placeholder="" style=" width:40px" readonly/></td>';
			   ...
			   if(kardex==1)html+='<td class="art'+cont+'" align="center"><input class="uk-form-small art'+cont+'" id="iva_'+cont+'" type="text" name="iva_'+cont+'" size="5" maxlength="5" value=\"'+r[3]+'\" onkeyup="valor_t('+cont+');" style=" width:50px" readonly=""/></td>';
			   
			   else html+='<td class="art'+cont+'" align="center"><input class="uk-form-small art'+cont+'" id="iva_'+cont+'" type="text" name="iva_'+cont+'" size="5" maxlength="5" value=\"'+r[3]+'\" onkeyup="valor_t('+cont+');" onBlur="'+Fname+'('+cont+');" style=" width:50px" /></td>';
		...
		if(carros_rutas==1)html+='<td class="art'+cont+'"><input class="uk-form-small art'+cont+'"  value="'+Uni+'" type="text" id="'+cont+'cant_dev" name="cant_dev'+cont+'" placeholder=""  onKeyUp="'+Fname+'('+cont+');"/>';
			   ...
			   //valor unitario/val Min
			   if(r[13]==0){html+='<td class="art'+cont+'" ><input class="uk-form-small art'+cont+'" id="val_u'+cont+'" name="val_uni'+cont+'" type="text" onkeyup="puntoa($(this));keepVal('+cont+');valor_t('+cont+');" value=\"'+puntob(parseFloat(vsi))+'\"  onBlur="change16('+cont+');valMin($(this),'+r[6]+','+vsi+','+cont+');'+Fname+'('+cont+');" readonly="" style=" width:70px"/><input class="uk-form-small art'+cont+'" id="val_u'+cont+'HH" name="val_u'+cont+'" type="hidden"  value="0" /><input class="uk-form-small art'+cont+'" id="val_u'+cont+'H" name="val_u'+cont+'H" type="hidden" readonly="" value="0" /><input class="uk-form-small art'+cont+'" id="valMin'+cont+'" type="hidden" name="valMin'+cont+'" size="5" maxlength="5" value="'+r[6]+'" style=" width:30px"/><input class="uk-form-small art'+cont+'" id="val_orig'+cont+'" name="val_orig'+cont+'" type="hidden"  value=\"'+puntob(parseFloat(vsi))+'\" /><input class="uk-form-small art'+cont+'" id="val_origb'+cont+'" name="val_origb'+cont+'" type="hidden"  value=\"'+parseFloat(r[4])+'\" /><input class="uk-form-small art'+cont+'" id="val_cre'+cont+'H" name="val_cre'+cont+'H" type="hidden" readonly="" value="'+r[20]+'" /></td>';
			 }
			 else{html+='<td class="art'+cont+'" ><input class="uk-form-small art'+cont+'" id="val_u'+cont+'" name="val_uni'+cont+'" type="text" onkeyup="puntoa($(this));keepVal('+cont+');valor_t('+cont+');" value=\"'+puntob(parseFloat(vsi))+'\"  onBlur="valMin($(this),'+r[6]+','+vsi+','+cont+');change16('+cont+');'+Fname+'('+cont+');" style=" width:70px"/><input class="uk-form-small art'+cont+'" id="val_u'+cont+'HH" name="val_u'+cont+'" type="hidden"  value="0" /><input class="uk-form-small art'+cont+'" id="val_u'+cont+'H" name="val_u'+cont+'H" type="hidden"  value="0" /><input class="uk-form-small art'+cont+'" id="valMin'+cont+'" type="hidden" name="valMin'+cont+'" size="5" maxlength="5" value="'+r[6]+'" style=" width:30px"/><input class="uk-form-small art'+cont+'" id="val_orig'+cont+'" name="val_orig'+cont+'" type="hidden"  value=\"'+puntob(parseFloat(vsi))+'\" /><input class="uk-form-small art'+cont+'" id="val_origb'+cont+'" name="val_origb'+cont+'" type="hidden"  value=\"'+parseFloat(r[4])+'\" /><input class="uk-form-small art'+cont+'" id="val_cre'+cont+'H" name="val_cre'+cont+'H" type="hidden" readonly="" value="'+r[20]+'" /></td>';}
...
function addRow2(r,Fname,EliF)
{
	...
	//ref
			  ...
			  if(usarTalla==1)html+='<td class="art'+cont+'" align="center"><input class="uk-form-small art'+cont+'"  value="'+r[9]+'" type="text" id="talla'+cont+'" name="talla'+cont+'" placeholder="" style=" width:40px"  onBlur="'+Fname+'('+cont+');"/></td>';	
			   ...
			   else html+='<td class="art'+cont+'" align="center"><input class="uk-form-small art'+cont+'" id="iva_'+cont+'" type="text" name="iva_'+cont+'" size="5" maxlength="5" value=\"'+r[3]+'\" onkeyup="valor_t('+cont+');" style=" width:50px" onBlur="'+Fname+'('+cont+');"/></td>';
		...
		if(carros_rutas==1)html+='<td class="art'+cont+'"><input class="uk-form-small art'+cont+'"  value="'+Uni+'" type="text" id="'+cont+'cant_dev" name="cant_dev'+cont+'" placeholder=""  onKeyUp="'+Fname+'('+cont+');"/>';
			   ...
			   //valor unitario/val Min
			   if(r[13]==0){html+='<td class="art'+cont+'" ><input class="uk-form-small art'+cont+'" id="val_u'+cont+'" name="val_uni'+cont+'" type="text" onkeyup="puntoa($(this));keepVal('+cont+');valor_t('+cont+');" value=\"'+puntob(parseFloat(vsi))+'\"  onBlur="change16('+cont+');valMin($(this),'+r[6]+','+vsi+','+cont+');" readonly="" style=" width:70px"/><input class="uk-form-small art'+cont+'" id="val_u'+cont+'HH" name="val_u'+cont+'" type="hidden"  value="0" /><input class="uk-form-small art'+cont+'" id="val_u'+cont+'H" name="val_u'+cont+'H" type="hidden" readonly="" value="0" /><input class="uk-form-small art'+cont+'" id="valMin'+cont+'" type="hidden" name="valMin'+cont+'" size="5" maxlength="5" value="'+r[6]+'" style=" width:30px"/><input class="uk-form-small art'+cont+'" id="val_orig'+cont+'" name="val_orig'+cont+'" type="hidden"  value=\"'+puntob(parseFloat(vsi))+'\" /><input class="uk-form-small art'+cont+'" id="val_origb'+cont+'" name="val_origb'+cont+'" type="hidden"  value=\"'+parseFloat(r[4])+'\" /><input class="uk-form-small art'+cont+'" id="val_cre'+cont+'H" name="val_cre'+cont+'H" type="hidden" readonly="" value="'+r[20]+'" /></td>';
			 }
			 else {html+='<td class="art'+cont+'" ><input class="uk-form-small art'+cont+'" id="val_u'+cont+'" name="val_uni'+cont+'" type="text" onkeyup="puntoa($(this));keepVal('+cont+');valor_t('+cont+');" value=\"'+puntob(parseFloat(vsi))+'\"  onBlur="valMin($(this),'+r[6]+','+vsi+','+cont+');change16('+cont+');'+Fname+'('+cont+');" style=" width:70px"/><input class="uk-form-small art'+cont+'" id="val_u'+cont+'HH" name="val_u'+cont+'" type="hidden"  value="0" /><input class="uk-form-small art'+cont+'" id="val_u'+cont+'H" name="val_u'+cont+'H" type="hidden"  value="0" /><input class="uk-form-small art'+cont+'" id="valMin'+cont+'" type="hidden" name="valMin'+cont+'" size="5" maxlength="5" value="'+r[6]+'" style=" width:30px"/><input class="uk-form-small art'+cont+'" id="val_orig'+cont+'" name="val_orig'+cont+'" type="hidden"  value=\"'+puntob(parseFloat(vsi))+'\" /><input class="uk-form-small art'+cont+'" id="val_origb'+cont+'" name="val_origb'+cont+'" type="hidden"  value=\"'+parseFloat(r[4])+'\" /><input class="uk-form-small art'+cont+'" id="val_cre'+cont+'H" name="val_cre'+cont+'H" type="hidden" readonly="" value="'+r[20]+'" /></td>';}
...
//alert('entra if ;'+$sel.length);
        $.ajax({
		url:'ajax/add_art_ven.php',
		data:{codBar:$cod=$('#cod').val(),fe_ven:$('#feVen').val(),ref:$('#Ref').val(),idCli:''+$('#ced').val()+'',tc:tipoCli,co:cotiza},
	    type:'POST',
		dataType:'text',
		success:function(text){
		var resp=text;
...
contTO=0;
	...
	if(confirm('Existen otras Ref. con Fecha de Vencimiento diferente, Desea buscar otras Referencias??'))
			   ...
			   //alert('nrm');
			   //alert('Pvp Cre:'+r[20]);
			 var html=addRow(r,"save_remi","eli_remi_mod");
...
//move($('#'+cont+'cant_'),$('#'+(cont-1)+'cant_'),$('#'+(cont+1)+'cant_'),$('#'+cont+'cant_'),$('#unidades'+cont));
			  ...
			  // $cod.prop('value',r[21]);  
			   ...
			   var html=addRow2(r,"save_remi","eli_remi_mod");
...
function dctoB()
{
	...
	if(totItems!=0)p=redondeox(D/totItems,2);
	else p=D;
...
function serv($selc,Fname,EliF)
{
	...
	// ID SERV
	html+='<td class="art'+cont+'" align="center"><input class="uk-form-small art'+cont+' col_id_serv"  value="'+r[0]+'" type="text" id="id_serv'+cont+'" name="id_serv'+cont+'" placeholder=""   onChange="'+Fname+'('+cont+')"/><input class="uk-form-small art'+cont+'" id="'+cont+'cant_" type="hidden" name="cant_'+cont+'" size="5"   value="'+1+'" style=" width:50px" readonly=""/></td>';
	...
	//  NOTA
	html+='<td class="art'+cont+'"><textarea style=" width:170px" cols="10" rows="1" class="art'+cont+' col_nota_serv" name="nota'+cont+'" id="nota'+cont+'" ></textarea></td>';
...
html+='<td class="art'+cont+'" ><input class="uk-form-small art'+cont+' col_pvp_serv" id="val_u'+cont+'" name="val_serv'+cont+'" type="text" onkeyup="puntoa($(this));keepVal('+cont+');valor_t('+cont+');" value=\"'+puntob(vsi)+'\"  onBlur="change16('+cont+');valMin($(this),'+r[3]+','+vsi+','+cont+');'+Fname+'('+cont+');"  /><input class="uk-form-small art'+cont+'" id="val_u'+cont+'HH" name="val_u'+cont+'" type="hidden"  value="0" /><input class="uk-form-small art'+cont+'" id="val_u'+cont+'H" name="val_u'+cont+'H" type="hidden"  value="0" /><input class="uk-form-small art'+cont+'" id="valMin'+cont+'" type="hidden" name="valMin'+cont+'" size="5" maxlength="5" value="'+r[6]+'" style=" width:30px"/><input class="uk-form-small art'+cont+'" id="val_orig'+cont+'" name="val_orig'+cont+'" type="hidden"  value=\"'+puntob(parseFloat(vsi))+'\" /><input class="uk-form-small art'+cont+'" id="val_origb'+cont+'" name="val_origb'+cont+'" type="hidden"  value=\"'+parseFloat(r[4])+'\" /><input class="uk-form-small art'+cont+'" id="val_cre'+cont+'H" name="val_cre'+cont+'H" type="hidden" readonly="" value="'+r[4]+'" /></td>';
			   // val sub_tot
			   html+='<td class="art'+cont+'"><input class="uk-form-small art'+cont+' col_sub_tot_serv" id="val_t'+cont+'" name="val_tot'+cont+'" type="text" readonly="" value="0" style=" width:70px"/><input class="uk-form-small art'+cont+'" id="val_t'+cont+'HH" name="val_t'+cont+'" type="hidden" readonly="" value="0" /><input class="uk-form-small art'+cont+'" id="val_t'+cont+'H" name="val_t'+cont+'H" type="hidden" readonly="" value="0" /></td>';
...
function change16(i)
{
	...
	var FormaPago=val_ele('#form_pago');
	var $val_u=$('#val_u'+i);
	var val_u=quitap($val_u.val())*1.16;
		...
		success:function(text){
			...
			if(!esVacio(text)){
				...
				location.assign("fac_remi.php?resp=imp&tipoFAC="+val_ele('#tipoFAC'));	
					...
					location.assign("fac_remi.php?resp=mod&tipoFAC="+val_ele('#tipoFAC'));	
					}else{alert('ERROR, intentelo mas tarde');}
				}
